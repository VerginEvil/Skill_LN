...
>
> Source: Infor LN Public Interfaces & Process Extensions Reference Guide (Cloud), pp. 1350-1351

```baan
DLL:   tsextsocapi
This function is available from 2020.03 (KB2111387).
Syntax: long Service.GetServiceOrderSettings(
...
... )
Usage:        Expl:   This function retrieves service orders settings. Individual
fields are retrieved, multiple fields can be retrieved
in one call.
This function is Multi Site aware, and data is read from general
level (parameter table tssoc000), or office or site level
(settings table tssoc003) depending on the specified input
...
Example: when use margin control unit and lower margin
...
|* Variable arguments:
"sqmc",                   --> input
use.margin.control,       --> output
"lwgm",                   --> input
lower.margin) <> 0 then --> output
|* Error, do something
