...
>
> Source: Infor LN Public Interfaces & Process Extensions Reference Guide (Cloud), pp. 1344-1345

```baan
DLL:   tsextsocapi
This function is available from 2020.03 (KB2111387).
Syntax: long Service.GetMobileServiceSettings(
...
... )
Usage:        Expl:   This function retrieves mobile service settings. Individual
fields are retrieved, multiple fields can be retrieved
in one call.
This function is Multi Site aware, and data is read from general
level (parameter table tssoc002), or office or site level
(settings table tssoc004) depending on the specified input
...
Example: when issue material and distribute visist report
are needed, the function must be called as follows:
if service.GetMobileServiceSettings(
|* Fixed arguments:
company,                  --> input
...
|* Variable arguments:
"issm",                   --> input
issue.material,           --> output
"dmvr",                   --> input
distribute.visit.report) <> 0 then --> output
|* Error, do something
