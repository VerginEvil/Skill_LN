...
>
> Source: Infor LN Public Interfaces & Process Extensions Reference Guide (Cloud), pp. 1328-1329

```baan
DLL:   tsextcmmapi
This function is available from     2020.03 (KB2111387  ).
Syntax: long Service.GetClaimSettings(
...
... )
Usage:        Expl:   This function retrieves claim settings. Individual
fields are retrieved, multiple fields can be retrieved
in one call.
This function is Multi Site aware, and data is read from general
level (parameter table tscmm000), or office or site level
(settings table tscmm003) depending on the specified input
...
Example: when ATP check enabled and default task
are needed, the function must be called as follows:
if service.GetClaimSettings(
|* Fixed arguments:
company,                                                --> input
...
|* Variable arguments:
"atpr",                                                 --> input
atp.check.enabled,                                      --> output
"ctsk",                                                 --> input
default.task) <> 0 then                               --> output
|* Error, do something
