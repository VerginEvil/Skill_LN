...
>
> Source: Infor LN Public Interfaces & Process Extensions Reference Guide (Cloud), pp. 1335-1337

```baan
DLL:   tsexteppapi
This function is available from     2020.03 (KB2111387  ).
Syntax: long Service.GetQuoteSettings(
...
... )
Usage:        Expl:   This function retrieves Quote settings. Individual
fields are retrieved, multiple fields can be retrieved
in one call.
This function is Multi Site aware, and data is read from general
level (parameter table tsepp000), or office or site level
(settings table tsepp003) depending on the specified input
...
Example: when quote duration and proposal method
are needed, the function must be called as follows:
if service.GetQuoteSettings(
|* Fixed arguments:
company,                                                --> input
...
|* Variable arguments:
"qttm",                                                 --> input
quote.duration,                                         --> output
"prmd",                                                 --> input
proposal.method) <> 0 then                               --> output
|* Error, do something
