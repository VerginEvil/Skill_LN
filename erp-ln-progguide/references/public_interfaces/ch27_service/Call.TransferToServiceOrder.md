...
can be transferred, it is not possible to transfer the call to
one of these service orders as it is not possible to make a
selection. When iCreateNewOrderWhenMultipleOpenOrdersPresent is
...
When multiple open service orders are present to which
the call can be transferred, it is not possible to
transfer the call to one of these service orders as it
is not possible to make a selection.
When iCreateNewOrderWhenMultipleOpenOrdersPresent is set
to Yes a new service order and activity line will be
generated.
When iCreateNewOrderWhenMultipleOpenOrdersPresent is set
to No, the call will not be transferred to service
order.
(mandatory Yes/No)
iSetStatusToPlanned
