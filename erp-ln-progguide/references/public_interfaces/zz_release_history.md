# Appendix A Public Interfaces and Process Extensions per Infor LN Cloud release

> Chapter: Appendix
>
> Group: -
>
> Source: Infor LN Public Interfaces & Process Extensions Reference Guide (Cloud), pp. 69-2361

## Public Interfaces

The following table shows the Infor LN Cloud releases in which new Public Interfaces were released:

### 2019.01

- BOD.ExecuteMethod
- BOD.ExecuteOnAcknowledgeForChameleon
- BOD.ExecuteOnLoadForChameleon
- BOD.ExecuteOnProcessForChameleon
- BOD.ExecuteOnSyncForChameleon
- BOD.ExecuteOnUpdateForChameleon
- BOD.ExecutePublishForChameleon
- BOD.ExecuteShowForChameleon
- BOD.GetIdAccountingEntity
- BOD.GetIdLocation
- BOD.GetIdLogicalID
- BOD.Publish
- Common.ConvertAmount
- Common.GenerateFirstFreeNumber
- Common.GetFinancialCompanyOfEntity
- Common.GetFormattedAddress
- Common.GetISOCodeOfLNLanguage
- Common.RoundAmount
- Common.RoundQuantity
- Exception.Delete
- Exception.GetMessage
- Exception.NumberOfMessages
- Item.GetPurchaseData
- Item.GetSalesData
- Item.GetServiceData
- Item.GetSupplier
- Item.GetWarehousingData

### 2019.03

- BOD.ActionsAfterProcessingIncomingRequest
- BOD.ActionsAfterProcessingIncomingRequestWithAutomaticProcessing
- BOD.ActionsBeforeProcessingIncomingRequest
- BOD.HandleStagingAfterProcessingIncomingRequest
- BOD.HandleStagingBeforeProcessingIncomingRequest
- BOD.PublishLNMessage
- Common.ConvertQuantity
- Item.GetOrderingData

### 2019.04

- BOD.ConvertFromERPItem
- BOD.ConvertToERPItem
- Item.GetData
- Pricing.SimulatePurchasePrice
- Pricing.SimulateSalesPrice
- PurchaseContract.GetTotalAmounts

### 2019.05

- FreightOrderCluster.ConfirmDelivery
- Load.ConfirmDelivery
- Shipment.ConfirmDelivery

### 2019.06

- BDE.ExecuteMethod

### 2019.08

- AdjustmentOrder.Process
- Common.GetParameters
- FreightPlanning.ActualizePlan
- FreightPlanning.GenerateForSpecificOrderSelection
- FreightPlanning.MoveShipmentToLoad
- Item.GetCostingData

### 2019.12

- Pricing.RetrievePriceBookDataForItem
- Purchase.GenerateRetrobilledPriceChangeAdvices
- Purchase.GenerateRetrobilledPriceChangeAdvicesForContracts
- Sales.GenerateRetrobilledPriceChangeAdvices

### 2020.01

- Common.GetCurrencyRates
- Item.ConvertPlanItemToItem
- Item.GetPlanningTimeFenceDate

### 2020.03

- AdjustmentOrder.StartDetail
- AdjustmentOrder.StartMultiMain
- AdjustmentOrder.StartOverview
- CycleCountingOrder.StartDetail
- CycleCountingOrder.StartMultiMain
- CycleCountingOrder.StartOverview
- Item.GetProductionData
- ItemMasterPlan.StartPlan
- ItemPurchase.StartDetail
- ItemPurchase.StartMultiMain
- ItemPurchase.StartOverview
- ItemPurchaseBusinessPartner.StartDetail
- ItemPurchaseBusinessPartner.StartOverview
- ItemPurchaseBySite.StartDetail
- ItemRoutingOperations.StartOverview
- ItemWarehouseFixedLocation.StartOverview
- ItemWarehouseInventoryTransaction.StartDetail
- ItemWarehouseInventoryTransaction.StartOverview
- JobShopRouting.StartMultiMain
- LandedCosts.GetSettings
- MaterialPricing.GetPurchaseSettings
- MaterialPricing.GetSalesSettings
- PlannedInventoryTransaction.StartDetail
- PlannedInventoryTransaction.StartOverview
- PlannedOrders.StartOverview
- Pricing.GetGeneralSettings
- Pricing.GetPurchaseSettings
- Pricing.GetSalesSettings
- Pricing.GetServiceSettings
- Purchase.GetContractSettings
- Purchase.GetGeneralSettings
- Purchase.GetOrderSettings
- Purchase.GetRequisitionSettings
- Purchase.GetRFQSettings
- Purchase.GetScheduleSettings
- PurchaseContract.StartWorkbench
- PurchaseOrder.StartIntakeWorkbench
- PurchaseOrderLines.StartOverview
- Sales.GetContractSettings
- Sales.GetGeneralSettings
- Sales.GetOrderSettings
- Sales.GetQuoteSettings
- Sales.GetScheduleSettings
- Service.GetClaimSettings
- Service.GetGeneralSettings
- Service.GetMaintenanceSalesOrderSettings
- Service.GetMobileServiceSettings
- Service.GetPlanningSettings
- Service.GetQuoteSettings
- Service.GetServiceOrderSettings
- Service.GetWorkOrderSettings
- SimulatedPurchasePrices.StartOverview
- SiteItemInventory.StartDetail
- SiteItemInventory.StartOverview
- StockPoint.Move
- WarehouseItemInventory.StartDetail
- WarehouseItemInventory.StartMultiMain
- WarehouseItemInventory.StartOverview
- WarehouseOrder.StartDetail
- WarehouseOrder.StartMultiMain
- WarehouseOrder.StartOverview
- WarehouseReceipts.StartWorkbench
- Warehousing.GetSettings

### 2020.04

- CompanyItemInventory.StartDetail
- CompanyItemInventory.StartOverview
- SalesOrder.StartMultiMain
- Service.DetermineStartOrFinishTime
- Service.GetResourceCalendarCode
- ShipmentNotice.StartDetail
- ShipmentNotice.StartMultiMain
- ShipmentNotice.StartOverview
- Warehouse.StartDetail
- Warehouse.StartOverview

### 2020.05

- HandlingUnit.StartAutomaticInboundProcessing
- InboundAdvice.StartAutomaticInboundProcessing
- InboundInspection.StartAutomaticInboundProcessing
- InboundOrderLine.StartAutomaticInboundProcessing
- InboundOrderLineActivity.ModifyAutomaticProcessing
- InboundRun.StartAutomaticInboundProcessing
- Load.StartAutomaticOutboundProcessing
- OutboundAdvice.StartAutomaticOutboundProcessing
- OutboundOrderLine.StartAutomaticOutboundProcessing
- OutboundOrderLineActivity.ModifyAutomaticProcessing
- OutboundRun.StartAutomaticOutboundProcessing
- QuarantineInventory.StartAutomaticInboundProcessing
- QuarantineInventoryDispositionLine.Process
- QuarantineInventoryHandlingUnit.Process
- ReceiptLine.StartAutomaticInboundProcessing
- Shipment.StartAutomaticOutboundProcessing
- StorageList.StartAutomaticInboundProcessing

### 2020.06

- BillOfMaterialLines.StartOverview
- Calendar.PlanLeadTimeBackward
- Calendar.PlanLeadTimeForward
- Item.ConvertItemToPlanItem
- Item.StartDetail
- Item.StartMultiMain
- Item.StartOverview
- Item.StartWhereUsedComponent
- ItemBySite.StartDetail
- ItemBySite.StartOverview
- ItemOrderPlan.StartPlan
- ItemOrderPlan.StartPlanForProject
- ItemOrderPlan.StartPlanForProjectPeg
- JobShopBillOfMaterial.StartMultiMain
- JobShopBillOfMaterial.StartOverview
- Opportunity.StartMultiMain
- PlanItemExceptionMessageTotals.StartOverview
- ProductionOrders.StartOverview
- SalesQuote.StartMultiMain
- StandardCosts.CalculateForNewItem

### 2020.07

- Assembly.GetSettings
- Invoice.StartMultiMain
- JobShop.GetMasterDataSettings
- JobShop.GetOrderSettings
- ProductionOrder.InitiateInventoryIssue
- ProductionOrder.ReportProduct
- ProductionOrderReject.MoveToQuarantine
- PurchaseContractLine.GetNetAmountOnPriceDate
- PurchaseOrder.StartMultiMain
- Repetitive.GetSettings
- SalesOrderLine.Cancel
- StandardCosts.Calculate
- Tools.GetSettings

### 2020.08

- BusinessPartner.CheckOverdueInvoices
- BusinessPartner.GetCreditLimitAndBalances
- HandlingUnit.Create
- HandlingUnit.Link
- PackageDefinition.DetermineMultipleOfQuantity
- SalesOrderLine.CopyFromOriginalInvoice
- SalesOrderLine.CopyFromOriginalOrder
- SalesOrderLine.CopyFromOriginalShipment

### 2020.09

- FactoryTrackQuery.CreateElement
- FactoryTrackQuery.CreateRow
- FactoryTrackQuery.GetElement
- ProjectBid.StartMultiMain
- ProjectContract.StartMultiMain

### 2020.10

- CommissionsAndRebates.CalculateCumulativeSales
- CommissionsAndRebates.DetermineSalesOrderData
- CommissionsAndRebates.EvaluateAgreementGrades
- CommissionsAndRebates.InsertBasedOnAgreementGrades
- CommissionsAndRebates.InsertCumulativeSales
- PlannedPeggingRelations.FindSupplyForOrder
- ProductVariant.StartMultiMain
- SalesOrderLine.StartDetail
- SalesQuoteLine.StartDetail

### 2020.11

- DataAuthorization.DocumentActionAllowed
- DataAuthorization.DocumentFilter
- JobShopBillOfMaterial.ApproveRevision
- JobShopRouting.CreateNewRevision
- ProductionOrder.ResetCompletedToActive
- ProductionOrderReject.Scrap
- SalesOrderLine.ConfirmBackorderLine
- SalesOrderLine.ConfirmSalesDelivery
- WarehouseOrder.CreateReturn

### 2020.12

- AssemblyLineStationOrder.AllocateParts
- AssemblyLineStationOrder.ReportFinished
- Item.StartCopy
- JobShopBillOfMaterial.ValidateRevision
- JobShopRouting.ApproveRevision
- PurchaseContract.Activate
- PurchaseContract.Deactivate
- PurchaseContract.Terminate
- PurchaseContractLine.Activate
- PurchaseContractLine.Deactivate
- PurchaseContractLine.Terminate
- PurchaseContractLineLogisticData.Activate
- PurchaseContractLineLogisticData.Deactivate
- PurchaseContractPrice.Activate
- PurchaseContractPrice.Deactivate
- PurchasePayableReceipt.ChangePriceAndDiscountAfterReceipt
- Shipment.Confirm
- Tax.Calculate
- WorkOrder.Complete
- WorkOrder.Release

### 2021.01

- OrderInspection.Close
- OrderInspection.Complete
- OrderInspection.Process
- ProductionBillOfMaterial.ApproveRevision
- ProductionBillOfMaterial.CopyToJobShop
- SalesOrderInvoiceLine.ReleaseToInvoicing
- Shipment.Freeze

### 2021.03

- BusinessPartner.StartMultiMain
- BusinessPartner.StartOverview
- Catalog.StartOverview
- CommissionsAndRebates.InsertCumulativeSalesV2
- InboundOrderLine.Receive
- Inventory.DetermineQuantityAndValue
- ItemPurchaseBusinessPartner.CalculateLeadTimes
- ProductionOrder.CheckShortages
- ProductionOrder.Release
- SalesOrderLine.StartAutomaticProcessing

### 2021.04

- FactoryTrackBDE.CreateElement
- FactoryTrackBDE.CreateNode
- HandlingUnit.GenerateEmptyHandlingUnits
- HandlingUnit.PrintLabelByPackageLabelBOD
- JobShopBillOfMaterial.RemoveUseUp
- Lot.StartDetail
- Lot.StartOverview
- ProductionOrder.CreateAsBuilt
- ProductionOrderOperation.ReportProduct
- ProductionOrderOperation.SetStatusCompleted
- ReceiptLine.GenerateLots
- ReceiptLine.GenerateSerials
- SerialsByWarehouse.StartDetail
- SerialsByWarehouse.StartOverview

### 2021.05

- BusinessPartner.StartEasyEntry
- Calendar.GetPeriodCapacity
- Item.CreateSerialNumber
- JobShopBillOfMaterial.Copy
- JobShopBillOfMaterial.SetUseForCosting
- JobShopBillOfMaterial.SetUseForPlanning
- JobShopRouting.ClearUseForPlanning
- JobShopRouting.Copy
- JobShopRouting.SetUseForCosting
- JobShopRouting.SetUseForPlanning
- PurchaseRequisition.ApproveApprovalRecord
- PurchaseRequisitionLine.ConvertToPurchaseOrder
- PurchaseRequisitionLine.ConvertToPurchaseRFQ

### 2021.06

- AdjustmentOrderLine.Submit
- Inventory.GetTimePhasedAvailability
- ItemMasterPlan.Update
- PaymentAdvice.StartProcessPayments
- ProductionBillOfMaterial.ValidateRevision

### 2021.07

- Shipment.PrintLabelsByPackageLabelBOD
- ShipmentLine.PrintLabelsByPackageLabelBOD
- WorkOrder.GetActualRepairCosts

### 2021.08

- Assembly.CalculatePartRequirements
- HandlingUnit.ConfirmReceipt
- HandlingUnit.Unlink
- InboundAdvice.GetAvailableLocation
- Inventory.DetermineQuantity
- Inventory.DetermineQuantityAndValueV2
- Inventory.GetTimePhasedAvailabilityV2
- JobShopBillOfMaterial.ExpireRevision
- ProductionBillOfMaterial.ExpireRevision
- ProductionBillOfMaterial.StartMultiMain
- ReceiptLine.Confirm
- SalesSchedule.GetForecastQuantity
- ServiceOrder.GenerateActivitiesFromMasterRouting

### 2021.09

- Assembly.UpdateSegmentSchedules
- AssemblyLineStationOrder.ReportFinishedV2
- HandlingUnit.ApproveRemaining
- HandlingUnit.RejectRemaining
- HandlingUnitStockPointDetail.Inspect
- Pricing.CalculateAmounts
- ProductionOrder.Cancel
- QuarantineInventory.Move
- QuarantineInventoryDispositionLine.Move
- QuarantineInventoryHandlingUnit.Move
- StandardCosts.CalculateAndGetStandardCost
- WarehouseInspection.Process
- WarehouseInspectionLine.Inspect

### 2021.10

- HandlingUnitStructure.GetBottomLevel
- HandlingUnitStructure.GetComponentLevel
- HandlingUnitStructure.GetInventoryLevel
- HandlingUnitStructure.GetOrderLineLevel
- HandlingUnitStructure.GetWholeTree
- Inventory.GetAnonymousInventoryQuantities
- Item.GetLotSerialRegistration
- ItemOrderPlan.Get
- NegativeInventory.Consume
- ProductionOrderOperation.SetRemainderStartDate
- Project.Activate

### 2021.11

- HandlingUnit.MoveToLoad
- HandlingUnit.MoveToShipment
- JobShopBillOfMaterial.CreateNewRevision
- JobShopRouting.CopyOperations
- Load.Freeze
- Load.Reopen
- ProductVariant.RegenerateJobShopStructure
- PurchaseOrderLine.CalculatePlannedReceiptDate
- SalesOrder.Approve
- SalesScheduleLine.Reprice
- Shipment.Reopen
- ShipmentLine.MoveToLoad
- ShipmentLine.MoveToShipment
- ShipmentLine.Split
- ShipmentNotice.ReceiveDirectDelivery
- StandardCosts.CalculateForSimulation
- WarehouseInspection.LinkHandlingUnit
- Weight.Convert
- WorkOrder.GenerateActivitiesFromMasterRouting

### 2021.12

- BusinessPartner.GetBuyFromData
- BusinessPartner.GetGeneralData
- BusinessPartner.GetInvoiceFromData
- BusinessPartner.GetInvoiceToData
- BusinessPartner.GetPayByData
- BusinessPartner.GetPayToData
- BusinessPartner.GetShipFromData
- BusinessPartner.GetShipToData
- BusinessPartner.GetSoldToData
- HandlingUnit.Close
- InspectionOrder.Print
- JobShopRouting.ExpireRevision
- ProductionOrder.ScrapRejectedMaterial
- ProjectEstimate.GenerateStructuralElements
- PurchaseOrder.StartAutomaticProcessing
- PurchaseOrderLine.Approve
- PurchaseOrderLine.ReleaseToWarehousing
- ServiceOrder.CheckAllMaterialLinesHaveBeenReceivedAndIssued
- ServiceQuote.Process

### 2022.01

- AdjustmentOrderLine.Process
- Common.ConvertTime
- HandlingUnit.GenerateLots
- HandlingUnit.GenerateSerials
- HandlingUnit.Receive
- ProductionOrder.StartInventoryOverview
- Project.Start360
- ProjectPCS.CalculateCost
- ReceiptLine.GenerateHandlingUnits
- SerializedItem.StartMultiMain
- ShipmentNoticeLine.Receive
- Tool.Request
- Tool.Return
- Tool.ReturnToOrder
- WorkOrder.StartMultiMain

### 2022.02

- Common.GetAdditionalEntityInfo
- Common.GetConversionFactor
- Item.ProjectPegAllowed
- Project.StartCopy
- ProjectPCS.Start360
- QuarantineInventoryDisposition.StartDetail
- QuarantineInventoryDisposition.StartOverview
- StockPoint.CheckBlocking
- WarehouseOrder.PrintLabelForActivity

### 2022.03

- Common.ConvertPrice
- Common.ValidateNumberGroupSeries
- JobShopBillOfMaterial.Explode
- PlannedActivity.SwitchStatus
- ProductionOrderOperation.ResetStatus
- StockPoint.PackHandlingUnits

### 2022.04

- Common.UpdateApprovedConversionFactor
- ProductionOrder.InitiateMaterialInventoryIssue
- ProductionOrder.ReportProductWithSerials
- ServiceOrder.Cancel
- ServiceOrder.Complete
- ServiceOrder.Release
- ServiceOrderActivity.Cancel
- ServiceOrderActivity.Complete
- ServiceOrderActivity.Release
- UnexpectedReceipt.Create
- WorkOrder.Cancel

### 2022.05

- PlannedOrders.TransferOrderSet
- ProductionOrder.ReportProductV2
- ProductionOrder.ReportProductWithSerialsV2
- ProductionOrderOperation.ReportProductV2
- ReceiptLine.Split
- ReceiptLine.SplitForLotsBatches
- ReceiptLine.SplitForSerials
- SalesOrder.ReleaseBlocking
- SalesOrderLine.ReleaseBlocking
- SalesOrderLineComponent.ReleaseBlocking
- ServiceOrderActivity.ProcessReturnDeliveries
- Tax.GetRate
- WarehouseInspectionLine.StartDetail
- WarehouseInspectionLine.StartOverview
- WarehouseOrderLine.Activate

### 2022.06

- BOD.ExecuteRequest
- HandlingUnit.Unpack
- ProductionOrder.InitiateInventoryIssueV2
- ProjectPCS.CopyProject
- PurchaseOrderLine.Cancel
- PurchaseOrderReceipt.StartDetail
- PurchaseSchedule.GenerateReleaseForPushAndPullForecast
- PurchaseSchedule.Terminate
- ServiceOrder.Cost
- ServiceOrderActivity.Cost
- ServiceOrderCostLine.Cost
- ShipmentLine.Confirm

### 2022.07

- AssemblyOrder.Create
- Load.StartDetail
- Load.StartMultiMain
- Load.StartOverview
- OutboundAdvice.StartDetail
- OutboundAdvice.StartGenerate
- OutboundAdvice.StartOverview
- PlannedOrders.StartGenerate
- ProductionOrderOperation.BackflushMaterialAndHours
- QuarantineInventory.StartDetail
- QuarantineInventory.StartOverview
- SalesContract.GetTotalAmounts

### 2022.08

- AssemblyOrder.CreateV2
- ConfigurableItemConstraintIDs.StartOverview
- ConfigurableItemGenericBillOfMaterial.StartOverview
- ConfigurableItemGenericPriceLists.StartOverview
- ConfigurableItemGenericRouting.StartOverview
- ConfigurableItemProductFeatures.StartOverview
- ConfigurableItemSettingsForDataGeneration.StartOverview
- CorrectiveActionPlan.SetStatus
- CorrectiveActionPlan.StartMultiMain
- CorrectiveActionPlanTask.SetStatus
- FRACASHeader.StartMultiMain
- NonConformanceReport.StartDetail
- PLMObject.CheckIn
- PLMObject.CheckOut
- PLMObject.UndoCheckIn
- PLMObject.UndoCheckOut
- PriceBookLines.StartOverview
- ReceiptLine.GenerateHandlingUnitsV2
- Shipment.MoveToLoad
- Shipment.StartDetail
- Shipment.StartMultiMain
- Shipment.StartOverview
- StandardTestProcedure.StartMultiMain
- TestingCombinations.StartOverview

### 2022.09

- ProductionOrder.MoveRejectedMaterialToQuarantine

### 2022.10

- CycleCountingOrder.Generate
- CycleCountingOrder.Process
- PhysicalBreakdown.Change
- ProformaInvoice.StartWorkbench
- ProjectActivityBudget.AddCPQConfiguration
- ProjectContractDeliverable.AddCPQConfiguration
- ServiceOrder.GetTotalSalesAndCostAmounts
- ServiceOrder.ReadShippingAddressForCostLine
- ServiceOrder.StartMultiMain
- ServiceOrderActivity.StartMultiMain
- SupplierClaimLine.Approve
- SupplierClaimLine.Reject
- WorkOrder.DetermineStartOrFinishTime

### 2022.11

- AssemblyProductVariant.CalculateStandardCost
- Attendance.DeleteActualAttendance
- Attendance.DeleteActualAttendanceGenerated
- Attendance.DeleteActualAttendanceManuallyAdded
- Attendance.GetActualAttendance
- Attendance.GetActualAttendanceForDate
- Attendance.GetActualHours
- Attendance.GetActualHoursForDate
- Attendance.GetAttendanceTypeAbsence
- Attendance.GetAttendanceTypeAttendanceTime
- Attendance.GetAttendanceTypeAutoBreak
- Attendance.GetAttendanceTypeBasicShiftDifference
- Attendance.GetAttendanceTypeClosingDifference
- Attendance.GetAttendanceTypeNormalTime
- Attendance.GetAttendanceTypeOffsite
- Attendance.GetAttendanceTypePostingDifferenceClockIn
- Attendance.GetAttendanceTypePostingDifferenceClockOut
- Attendance.GetAttendanceTypePunchedBreak
- Attendance.GetAttendanceTypeStandbyTime
- Attendance.GetAttendanceTypeTravel
- Attendance.GetBalanceBeginningHours
- Attendance.GetBalanceBeginningOvertime
- Attendance.GetBalanceBeginningVacation
- Attendance.GetBalanceEndHours
- Attendance.GetBalanceEndOvertime
- Attendance.GetBalanceEndVacation
- Attendance.GetBalanceHours
- Attendance.GetBalanceHoursForDate
- Attendance.GetDate
- Attendance.GetDayOfMonth
- Attendance.GetDays
- Attendance.GetDaysForDate
- Attendance.GetDayType
- Attendance.GetDepartment
- Attendance.GetEmployee
- Attendance.GetEmployeeGroup
- Attendance.GetFirstDayOfPeriod
- Attendance.GetHours
- Attendance.GetHoursForDate
- Attendance.GetLastDayOfPeriod
- Attendance.GetMonth
- Attendance.GetNumberOfStartTransactions
- Attendance.GetNumberOfStartTransactionsForKindOfTime
- Attendance.GetNumberOfStopTransactions
- Attendance.GetNumberOfStopTransactionsForKindOfTime
- Attendance.GetNumberOfTransactions
- Attendance.GetNumberOfTransactionsForKindOfTime
- Attendance.GetOvertimeHours
- Attendance.GetOvertimeHoursForDate
- Attendance.GetPaidHours
- Attendance.GetPaidHoursForDate
- Attendance.GetPeriodNumber
- Attendance.GetPeriodTableCode
- Attendance.GetPeriodYear
- Attendance.GetPlannedHours
- Attendance.GetPlannedHoursForDate
- Attendance.GetVacationDays
- Attendance.GetVacationDaysForDate
- Attendance.GetWeek
- Attendance.GetWeekDay
- Attendance.GetYear
- Attendance.GetYearDay
- Attendance.IsFirstDayOfPeriod
- Attendance.IsLastDayOfPeriod
- Attendance.NewRemark
- Attendance.SetActualAttendance
- Attendance.SetDayType
- Attendance.UpdateActaulAttendance
- Attendance.UseAttendanceTypeAbsence
- Attendance.UseAttendanceTypeAttendanceTime
- Attendance.UseAttendanceTypeAutoBreak
- Attendance.UseAttendanceTypeBasicShiftDifference
- Attendance.UseAttendanceTypeOffSite
- Attendance.UseAttendanceTypePostingDifferenceTypeClockIn
- Attendance.UseAttendanceTypePostingDifferenceTypeClockOut
- Attendance.UseAttendanceTypePunchedBreak
- Attendance.UseAttendanceTypeStandbyTime
- Attendance.UseAttendanceTypeTravel
- BOD.InterruptPublishing
- BOD.ResumePublishing
- BusinessPartner.BankAccountByPayToSubmit
- HoldReasons.StartOverview
- MaintenanceSalesOrder.GenerateSerializedItem
- PlannedProductionOrder.Transfer
- SalesOrder.Block
- SalesOrderLine.Block
- SalesOrderLineComponent.Block
- ServiceOrderMaterialCosts.ProcessReturnDeliveries

### 2022.12

- Commissions.StartOverview
- Installment.Transfer
- ProductVariant.GenerateStructure
- ProductVariant.StartConfigurator
- ProjectActivity.SetInsertMode
- Rebates.StartOverview
- SalesOrders.StartOverview

### 2023.01

- AssemblyPart.StartProcessReturn
- Call.TransferToServiceOrder
- Common.ConvertAmountToWords
- CustomerClaim.Close
- InventoryCommitment.CreateAllocationBuffer
- Item.InsertSerialNumber
- NonConformanceReport.SetStatus
- OutboundOrderLineActivity.Create
- PurchaseOrderAdvice.Transfer
- Shipment.StartPrintPickAndLoadSheet

### 2023.02

- BuyFromBusinessPartner.StartOverview
- Contact.StartOverview
- GeneralHours.StartOverview
- HandlingUnit.SetToNotShipped
- HandlingUnit.SetToShipped
- ItemLotSerialTransaction.GetData
- Lot.StartGlobalLotBlocking
- Lot.StartPrintLotBlocking
- LotInventoryByWarehouseAndItem.StartDetail
- LotInventoryByWarehouseAndItem.StartMultiMain
- LotInventoryByWarehouseAndItem.StartOverview
- OrderInventoryTransaction.StartOverview
- ProductionOrder.UpdateMaterialToIssueQuantity
- ProductionOrderHours.CloseActiveLine
- ProductionOrderHours.InsertActiveLine
- ProductionOrderHours.StartOverview
- ShipmentLine.StartDetail
- ShipmentLine.StartOverview
- ShipmentLineStockPointDetails.StartOverview
- SpecificationInventory.GetUnallocatedAvailableQuantity
- Workflow.DisableSubmit
- Workflow.EnableSubmit

### 2023.03

- AllocationChangeOrder.Generate
- BOD.ComposeLocationId
- ProductionOrder.NewVersion

### 2023.04

- AccountsPayable.DetermineDueDate
- BankTransaction.CreateBankReconciliation
- Call.GenerateSerializedItem
- CustomerClaim.GenerateSerializedItem
- JobShopTasks.StartOverview
- PLMItem.ClearChildItems
- PLMItem.GetChildItems
- PLMItem.SendToProductionBOM
- PLMItem.SendToReceivedJSBOM
- PLMObject.GenerateKeyNumber
- PLMProductItem.SendToReceivedJSBOM
- PriceBook.StartMultiMain
- ProductionOrder.StartPrintDocuments
- ProjectPeggedInventory.StartDetail
- ProjectPeggedInventory.StartOverview
- Purchase.GetServicesProcurementOrderSettings
- PurchaseRequisitionLine.StartDetail
- SalesContractLines.StartOverview
- SalesContracts.StartOverview
- SalesOffices.StartOverview
- SalesOrder.FinalizeBlocking
- SalesOrder.StartPrintAcknowledgement
- SalesOrderActualDeliveryLine.ReadWarehousingStatusDescription
- SalesOrderInvoiceLine.ReadInvoicingStatusDescription
- SalesOrderLine.CreateSalesDelivery
- SalesOrderLine.GetStatusDescription
- SalesOrderLine.ReleaseToWarehousing
- SalesPriceMatrices.StartOverview
- SerializedItem.GetActiveContracts
- SerializedItem.StartOverview
- ServiceOrder.GenerateSerializedItem
- ServiceQuote.GenerateSerializedItem
- StandardItemConfigurationsOptions.StartOverview
- SubcontractingModel.ApproveRevision
- SubcontractingModel.SetUseForCosting
- SubcontractingModel.SetUseForPlanning
- SubcontractingModel.ValidateRevision
- SupplierClaim.GenerateSerializedItem
- WorkCenters.StartOverview
- WorkOrder.GenerateSerializedItem
- WorkOrderActivity.ProcessReturnDeliveries
- WorkOrderMaterialCosts.ProcessReturnDeliveries

### 2023.05

- BOD.CreateCodeDefinitionReferenceNode
- BOD.CreateIDNode
- BOD.CreateLastModificationPersonNode
- BOD.CreateNoteNode
- BOD.GetPublishingAllowed
- BOD.GetTextNumberFromNoteNode
- BOD.GetValueFromNodeElement
- Call.TransferToServiceOrderV2
- Call.TransferToWorkOrder
- GeneralLedger.CheckDocumentIsBalanced
- HandlingUnit.RemoveEmptyHandlingUnits
- HandlingUnit.ValidatePackageDefinition
- LotAndSerialSet.StartOverview
- ProductionOrder.StartClose
- ProductionOrder.StartDetail
- ProductionOrder.StartMultiMain
- ProductionOrder.StartReportOrdersCompleted
- ProductionOrderGroup.StartMultiMain
- ProductVariant.RegenerateJobShopStructureV2
- ProductVariantOptions.StartOverview
- ProjectCostPegTransfer.Generate
- Sales.StartProcessProFormaInvoices
- SalesQuote.Copy
- SerializedItem.Supersede
- ServiceContract.Activate

### 2023.06

- CustomerClaimLine.Approve
- CustomerClaimLine.Reject
- OutboundAdvice.GenerateHandlingUnit
- OutboundAdvice.RemoveHandlingUnit
- ProductionOrder.StartReportOperationsCompleted
- ProductionWarehouseOrders.StartDetail
- SalesOrder.InitiateChangeRequest
- SalesOrderChangeRequest.Approve
- SalesOrderChangeRequest.Process
- SalesOrderLine.CopyFromOriginalScheduleShipment
- SerializedItem.GetSupplierWarrantyEndDate
- ServiceInspections.StartOverview
- ServiceOrder.Plan
- ServiceOrderActivity.Plan
- SupplierClaim.SetRMAReceived

### 2023.07

- AssetBook.CalculateRemainingDepreciationLife
- Calendar.GetWorkingDays
- PickingList.SplitAdvice
- ProductionBillOfMaterial.GenerateProductSubcontractingModel
- ProjectedShipment.StartGenerate
- Sales.CalculatePlannedReceiptDate
- StorageList.SplitAdvice

### 2023.08

- BOD.ConvertFromISOUnitToERP
- Common.ConvertLeadTimeToISODuration
- InventoryInspection.Generate
- ItemSalesBusinessPartner.StartDetail
- ItemSalesBusinessPartner.StartOverview
- PlannedActivity.TransferToFieldService
- ProductionOrderAdvice.StartDetail
- ProductionOrderAdvice.StartOverview
- ProjectCostPegTransfer.StartDetail
- ProjectCostPegTransfer.StartMultiMain
- ProjectCostPegTransfer.StartOverview
- ProjectCostPegTransferLine.StartDetail
- ProjectCostPegTransferLine.StartOverview
- PurchaseOrderLine.GetCurrentActivity
- SalesQuotes.StartOverview
- WarehouseOrder.CalculateDeliveryDate

### 2023.09

- Common.ConvertAmountWithRates
- CustomerClaim.Submit
- FinancialIntegration.GetDefaultLedgerAccountAndDimensions
- InspectionOrderLine.Complete
- Inventory.CalculateTimePhasedAvailability
- Lot.Generate
- PhysicalBreakdown.CreateFromProjectBreakdownStructure
- ProductionOrder.GenerateHandlingUnitForLineSideLabeling
- ProductionOrder.StartResetStatus
- ProductionOrderOperation.Block
- ProjectCostPegTransfer.CheckAllowedBetweenPegs
- ProjectCostPegTransfer.CheckInventoryAvailable
- ProjectCostPegTransferLine.Process
- SalesContract.StartWorkbench
- SalesOrder.CalculateAmounts
- SalesOrderLine.ExecuteTradeComplianceCheck
- ShipmentLine.Cancel
- StockPointInventory.StartDetail
- StockPointInventory.StartOverview
- WarehouseOrderLine.ActivityIsApplicable
- WarehouseOrderLine.ActivityIsMandatory
- WarehouseReceipt.StartDetail
- WarehouseReceipt.StartMultiMain
- WarehouseReceipt.StartOverview

### 2023.10

- AccountsReceivable.DetermineDueDate
- BOD.ConvertFromERPDateToISODate
- BOD.ConvertFromISODateToERP
- CustomerClaimLine.ApproveForInvoiceLinking
- InventoryCommitment.GetAvailableQuantityToCommit
- Item.GetRevision
- ItemPlanning.StartDetail
- ItemPlanning.StartOverview
- OutboundOrderLine.Cancel
- OutboundOrderLine.UndoCancel
- PlannedPeggingRelations.FindDemandForOrder
- PriceBookLines.StartDetail
- Product.StartMultiLevelProductionStructure
- ProjectAccounting.GlobalApprove
- ProjectContractDeliverable.StartDetail
- ProjectCostPegTransfer.Process
- ProjectedShipment.GenerateOutboundAdvice
- ProjectedShipment.ReleaseOutboundAdvice
- PurchaseOrderMaterialSupplyLine.CreateForItemSubcontracting
- PurchaseOrderMaterialSupplyLine.DeleteSupplyOrder
- PurchaseOrderMaterialSupplyLine.GenerateSupplyOrder
- ReceiptLine.GetLanguage
- ReceiptLineStockPointDetails.StartOverview
- RequestForQuote.StartMultiMain
- SalesQuote.Process
- SalesQuoteLine.DetermineAmounts
- ServiceContract.GenerateConfigurationLines
- ServiceOrder.IssueInventory
- Shipment.GenerateHandlingUnit
- ShipmentLine.Reopen
- ShipmentNotice.GenerateHandlingUnit
- ShipmentNoticeLine.GenerateHandlingUnit
- ShipmentNoticeLine.RemoveHandlingUnit
- SupplierStagePaymentLine.Release
- SupplierStagePaymentLines.StartDetail
- SupplierStagePaymentLines.StartOverview
- Workflow.Recall
- Workflow.Submit

### 2023.11

- Common.CheckIBANCode
- Common.ConvertAmountWithTargetRates
- CustomerClaim.StartMultiMain
- InboundRun.GenerateStorageList
- Item.Copy
- Item.CopySiteData
- Load.Confirm
- OutboundAdvice.GenerateHandlingUnitV2
- PaymentAdvice.StartPrintExceptionErrors
- PlanItem.StartATPHandling
- Pricing.GlobalUpdateOfPricesViaPriceBooks
- ProcessingOptionSet.Create
- ProcessingOptionSet.Delete
- ProductionOrder.ProcessReturn
- ProductionOrderOperation.GenerateSubcontractingPurchaseDocument
- PurchaseOrderReceipt.Confirm
- PurchaseSchedule.Regenerate
- Sales.CalculatePlannedDeliveryDate
- ServiceContract.ActivateV2
- ServiceContract.GetTotalSalesAmount
- ServiceOrder.Close
- Tax.GetExternalTaxDetails
- Tax.GetInternalTaxDetails
- WarehouseOrderLine.UpdatePlannedDates
- Warehousing.ConvertKindOfOrderToOrderOrigin
- Warehousing.ConvertOrderOriginToKindOfOrder
- Workflow.RecallByTypedObjectID

### 2023.12

- ActionDates.StartMultiMain
- BillableLine.CreateInvoice
- CalculatedItemCosts.StartMultiMain
- CalculatedItemCosts.StartOverview
- EngineeringItemRevision.ApproveByProduction
- EngineeringItemRevision.CopyToItem
- GeneralLedger.CheckBatchLineIsBalanced
- InventoryCommitment.AllowedForOrderLine
- InventoryCommitment.CalculateCommitmentDate
- InventoryCommitment.CommitInventoryToOrderLine
- InvoicingBatch.CreateInvoice
- ItemRoutings.StartOverview
- ProjectBudget.GenerateBudgetCostAnalysis
- Tax.ProcessTestScenario

### 2024.01

- Address.Create
- AsBuilt.StartMultiMain
- AsBuilt.StartOverview
- Common.CalculateDiscountAmounts
- ConversionFactor.StartOverview
- DeliveryNote.StartMultiMain
- EstimatedToolRequirement.StartDetail
- EstimatedToolRequirement.StartOverview
- InspectionOrder.StartMultiMain
- JobShopRouting.CopyV2
- LotItem.Convert
- ProductVariant.GenerateProductStructureWithoutProject
- PurchaseOrderLine.Split
- ReinspectionOrder.Create
- SerializedItem.Convert
- StockPointTransactions.StartDetail
- StockPointTransactions.StartOverview
- Tax.GetBusinessPartnerTaxRegistrationID
- UtilizationByWeek.StartOverview
- WorkCell.ReportProduct
- WorkCell.SendToInventory

### 2024.02

- InboundOrderLine.StartPrintGoodsReceivedNote
- Inventory.Start360
- NonConformanceReport.StartSplit
- ProductAvailability.GetDeliveryDate
- ProductionItemBySite.Start360
- ProductionOrder.Regenerate
- ProductionOrder.ReportProductWithHandlingUnits
- ProjectPlanning.GenerateNetworkPlanning
- SalesSchedule.StartMultiMain

### 2024.03

- Common.ConvertISODurationToSeconds
- EngineeringItemRevision.GenerateEBomCopyData
- ExchangeLogBatchLineLevel.StartOverview
- ExchangeScheme.NonRegularImport
- ExchangeScheme.RegularImport
- InboundOrderLine.PrintGoodsReceivedNote
- IntercompanyTradeOrder.PresentForWarehouseOrderLine
- OutboundRun.HandlePickingList
- PlanItem.Copy
- PlanItem.CopySourcingStrategies
- PlanItem.CopySupplyingRelations
- PlanItem.CopySupplyingStrategies
- Pricing.SimulateTransferPrice
- ProductionOrderOperation.GenerateSubcontractingPurchaseDocumentV2
- Project.Copy
- ProjectAccounting.ProcessTransactions
- ProjectCostingBreaksServiceActivities.StartOverview
- Shipment.PrintPackingSlip

### 2024.04

- Inventory.ChangeMAUC
- Invoice.Reprint
- ItemLotSerialTracking.PrintReceipts
- PlannedOrder.StartMultiMain
- PLMFile.Download
- PLMObject.SendToIDM
- Pricing.StartPriceSimulator
- ProductionOrder.GetActualQuantity
- ProductionOrder.GetInProcessQuantity
- ProductionOrder.PrintCheckList
- ProductionOrder.PrintOperationNote
- ProductVariant.StartOverview
- ProjectPCS.CopyCustomizedProductStructureToCustomizedStructure
- ProjectPCS.CopyCustomizedProductStructureToStandardStructure
- ProjectPCS.CopyStandardProductStructureToCustomizedStructure
- PurchaseOrder.InitiateChangeRequest
- PurchaseOrderChangeRequest.Approve
- PurchaseOrderChangeRequest.Process
- PurchaseOrderReceipt.ConfirmV2
- QualityResourceAssignment.Cancel
- QualityResourceAssignment.Complete
- QualityResourceAssignment.Start
- QualityResourceAssignment.Stop
- SalesOrder.CopyBOM
- Shipment.PrintPickAndLoadSheet
- StandardCosts.CalculateAndGetStandardCostV2
- SubcontractedOperations.StartDetail
- SubcontractedOperations.StartOverview

### 2024.05

- ItemMasterPlan.Generate
- PlannedOrder.Replan
- ProductionOrder.PrintCuttingList
- ProductionOrder.PrintInspectionNote
- ProductionOrder.PrintMaterialIssueNote
- ProductionOrder.PrintMaterialList
- ProductionOrder.PrintOrderCoveringNote
- ProductionOrder.PrintOrderDistribution
- ProductionOrder.PrintReceiptNote
- ProductionOrder.PrintRoutingSheet
- ProductionOrder.PrintSawingList
- ProductionOrder.PrintSerialNumbersList
- ProductionOrder.PrintSubcontractingNote
- ProjectContractDeliverable.SetStatus
- RentalOrder.Close
- RentalOrder.Generate
- SalesOrder.DetermineAdditionalCostLines

### 2024.06

- Inventory.ChangeMAUCCostDetails
- ItemOrderPlan.Generate
- PhysicalBreakdown.ItemExistsInBreakdownOfParent
- PlannedDistributionOrder.TransferToPurchaseOrder
- PlannedDistributionOrder.TransferToWarehouseOrder
- PlannedOrderDistribution.StartOverview
- PLMFolder.StartMultiMain
- PLMFolder.StartOverview
- ProjectCostTransactions.StartOverview
- ProjectFinancialTransactions.StartOverview
- PurchaseOrders.StartOverview
- SalesOrder.RecalculatePriceAndDiscounts
- SalesOrderLine.RecalculatePriceAndDiscounts
- ShipmentLine.ConfirmIsAllowed
- StandardCosts.StartCalculate
- WarehouseReceipt.PrintReceipts
- WorkCell.RejectProduct

### 2024.07

- AlternativeItem.StartDetail
- AlternativeItem.StartOverview
- Asset.GetDefaultLedgerAccountAndDimensions
- Asset.StartTransferAssets
- EngineeringBillOfMaterial.Copy
- EngineeringItemRevision.ApproveByEngineering
- FinancialIntegration.RetrieveContributionToPcsWip
- Installment.ApproveCorrections
- Installment.CreateCorrections
- Installment.TransferCorrections
- Invoice.GetInterimRevenueLedgerAccountAndDimensions
- ItemLotSerialTransactions.StartOverview
- ItemSupplyingRelationships.StartDetail
- ItemSupplyingRelationships.StartOverview
- PlanItem.IsAggregateItem
- PlannedCostPegTransfers.StartDetail
- PlannedCostPegTransfers.StartOverview
- PlannedPRPOrders.StartGenerate
- ProductionBillOfMaterial.CreateNewRevision
- ProjectContractLines.StartOverview
- ProjectPCS.StartGenerateStructureForProductVariant
- RequestForQuoteLines.StartOverview
- SourcingStrategy.StartDetail
- SourcingStrategy.StartOverview
- SpecialDemandByItem.StartPrint
- SupplyingRelationships.StartDetail
- SupplyingRelationships.StartOverview
- SupplyStrategy.StartOverview
- WarehouseLocation.StartDetail
- WarehouseLocation.StartOverview
- WarehouseLocation.StartRecalculateOccupation

### 2024.08

- Address.StartDetail
- Call.TransferToMaintenanceSalesOrderPartLine
- ConformanceDocumentationRegister.StartMultiMain
- ItemMasterPlan.GenerateDemandPlan
- ItemMasterPlan.GenerateInventoryPlan
- ItemPurchaseBusinessPartner.GetSafetyTime
- ItemWarehouse.GetHandlingUnitsInUse
- JobSession.ChangeInputValue
- Opportunities.StartOverview
- PlannedPRPWarehouseOrders.Approve
- PlannedPRPWarehouseOrders.Transfer
- PLMFile.View
- ProductionOrderOperation.UpdateEstimatedHours
- Project.StartMultiMain
- ProjectAdvanceInstallmentSettlementMapping.StartOverview
- ProjectAdvancePayments.StartOverview
- ProjectContractDeliverable.CalculatePlannedDeliveryOrReceiptDate
- ProjectInstallments.StartOverview
- ProjectInvoicing.StartProcessProFormaInvoices
- ProjectInvoicing.StartTransferTransactionsToInvoicing
- ProjectPCS.CalculateSurcharges
- PurchaseOrder.PrintPurchaseOrder
- SalesOrder.PrintAcknowledgement
- SalesOrderInstallments.Close
- SalesQuote.Approve
- SalesQuoteLine.RecalculatePriceAndDiscounts
- ServiceQuote.GenerateQuoteLinesForMasterRouting
- SubcontractingModel.Explode
- Warehouse.StartOverview

### 2024.09

- AllocationChangeOrder.Process
- BillableLine.UpdateStatus
- BillingCycle.GetInvoicedate
- ConfigurableItem.ReadCPQSettings
- Country.StartDetail
- CrossDockOrderLines.Print
- ExceptionMessages.StartOverview
- HandlingUnit.Transfer
- InspectionOrder.PrintTestData
- Invoice.CreditInvoiceLine
- ProductionOrderOperation.PlanOperation
- SalesOrder.ApproveOnly
- SalesOrderInvoiceLine.ChangePriceAndDiscounts
- Shipment.PrintPackingList
- StandardCosts.PrintMultilevelCostCalculation

### 2024.10

- ActiveSuppliersByPlanItem.StartOverview
- ActiveSupplySources.StartOverview
- Call.StartOverview
- CustomerClaim.StartOverview
- InventoryCommitment.CalculateCommitmentDateV2
- InvoiceToBusinessPartnerOpenEntries.StartOverview
- MaterialToIssueForProductionOrders.StartOverview
- PlanItem.CalculateFinishDateFromRequirementDate
- PlanItem.CalculateRequirementDateFromFinishDate
- PlanItem.StartCopyPlanItemsToPlanningClusters
- PlannedPeggingRelations.FindDemandForOrderV2
- PlannedPeggingRelations.FindSupplyForOrderV2
- PLMObject.CreateGenerationBulk
- PLMObject.FreezeGenerationBulk
- PLMObject.SendToIDMForProject
- PLMObject.SendToIDMForProjectActivity
- PLMObject.SendToIDMForRevisions
- ProductionOrder.FinishOperationAddition
- ProductionOrder.PrintCheckListV2
- ProductionOrder.PrintCuttingListV2
- ProductionOrder.PrintInspectionNoteV2
- ProductionOrder.PrintMaterialIssueNoteV2
- ProductionOrder.PrintMaterialListV2
- ProductionOrder.PrintOperationNoteV2
- ProductionOrder.PrintOrderCoveringNoteV2
- ProductionOrder.PrintOrderDistributionV2
- ProductionOrder.PrintReceiptNoteV2
- ProductionOrder.PrintRejects
- ProductionOrder.PrintRoutingSheetV2
- ProductionOrder.PrintSawingListV2
- ProductionOrder.PrintSerialNumbersListV2
- ProductionOrder.PrintSubcontractingNoteV2
- ProductionOrder.StartOperationAddition
- ProductVariant.StartValidateProductVariants
- ProjectActivities.StartOverview
- ProjectHoldback.StartOverview
- ProjectInvoicing.TransferTransactionsToInvoicing
- SalesPrice.CalculateSurcharges
- ServiceOrder.StartOverview
- ServiceQuote.StartOverview

### 2024.11

- AccountsReceivable.StartOverview
- Calendar.StartUpdateWorkingHours
- Calendar.UpdateWorkingHours
- CalendarWorkingHours.StartOverview
- EngineeringItemRevision.Finalize
- HandlingUnit.RepackInInventory
- InvoiceToBusinessPartner.StartDetail
- ItemSerial.SetStatus
- PlannedPRPWarehouseOrders.StartDetail
- PlannedPRPWarehouseOrders.StartOverview
- PlannedPurchaseOrder.Transfer
- ProductionOrder.Split
- ProductVariant.GenerateStructureV2
- ProformaInvoice.CancelInvoice
- Project.UpdateStatus
- ProjectPCS.PrintActualStandardCost
- RentalAgreement.Cancel
- RentalAgreement.Complete
- RentalAgreement.Cost
- RentalAgreement.Plan
- RentalAgreement.Release
- RentalOrder.Cancel
- RentalOrder.Complete
- RentalOrder.Cost
- RentalOrder.Plan
- RentalOrder.Release
- RentalOrderCostLine.Cost
- RentalUsageLine.Approve
- ShipFromBusinessPartner.StartOverview
- WorkCenter.GetCalendar

### 2024.12

- Account.CrossValidateDimensions
- Address.GetGPSData
- Call.TransferToCustomerClaim
- CustomerClaimLine.CalculateClaimAmount
- HandlingUnit.CalculateWeightAndDimensions
- Invoice.ComposePrintPostInvoices
- ItemSalesByOffice.StartOverview
- OrderPegging.StartSelect
- Pricing.SimulateServicePrice
- ProductionOrder.CalculateDeliveryDate
- ProductionOrder.CalculateStartDate
- ProductionOrder.RegenerateV2
- ProductionOrders.ProcessMaterialShortages
- ProductVariant.GenerateProductStructureWithoutProjectV2
- ProjectPCS.PrintEstimatedCost
- SalesOrder.DetermineDefaultPurchaseOrderData
- SalesOrder.GenerateStructure
- TimePhasedOrderPoint.GenerateOrders

### 2025.01

- Common.CalculateDueDates
- EquipmentProcurementOrder.Approve
- EquipmentProcurementOrder.StartAutomaticProcessing
- ProjectPhysicalProgress.CopyToActivityOrCostObject
- PurchaseOrderLine.ConfirmPotentialBackorder
- SubcontractingModel.CreateNewRevision
- SubcontractingProcurementOrder.Approve
- SubcontractingProcurementOrder.StartAutomaticProcessing
- WorkOrder.StartDetail
- WorkOrder.StartOverview

### 2025.02

- CorrectiveActionPlan.StartOverview
- HandlingUnitStockPointDetail.SplitForSerial
- ItemsByManufacturerPartNumber.StartOverview
- ProductAvailability.GetWhenAvailableSchedule

### 2025.03

- DeliveryNote.Print
- OutboundOrderLine.CreateAdvice
- ProductionOrder.ProcessMaterialQuantityChanges
- ProductVariant.CreateWithOptions
- ProductVariant.ProcessSalesConfiguration
- Run.DetermineNumber
- SalesOrderLines.ReleaseManualActivities.StartOverview
- SubcontractingModel.StartMultiMain
- SupplierClaimLine.Settle
- WarehouseInspection.CopyImage
- Warehousing.StartAutomaticInboundProcessing
- Warehousing.StartAutomaticOutboundProcessing

### 2025.04

- BankStatement.Validate
- BRA.BrazilianInvoice.PrintPostInvoicesByArray
- BRA.BrazilianInvoice.PrintPostInvoicesByRange
- BusinessObjectReference.Unpack
- InboundAdvice.StartDetail
- InboundAdvice.StartOverview
- Item.ChangeOutboundMethod
- PackageDefinition.Validate
- PlannedInventoryTransaction.Rebuild
- PlanningBillOfCriticalCapacities.Generate
- PlanningBillOfCriticalMaterial.Generate
- POL.BusinessPartner.StartVerifyOnline
- ProductVariant.ReprocessCPQConfiguration
- ProjectAccounting.GenerateInterimResults
- QualityUserProfile.StartOverview
- RemittanceAdvice.Apply
- RemittanceAdvice.Post
- SalesOrder.UnpackObjectReference
- SalesOrderLine.GetComponentAndSalesSequence
- ServiceOrder.UnpackObjectReference
- ServiceQuote.StartProcess
- Shipment.RecalculateWeight
- StandardCosts.Actualize
- Tax.AddAdjustment
- Tax.CalculateTaxBaseAmount
- TestData.EvaluateAlgorithm

### 2025.05

- Address.StartPrintByBusinessPartner
- AdjustmentOrder.GenerateHandlingUnits
- IntercompanyTradeOrder.PostTransactionLine
- MaintenanceSalesOrderLine.Release
- ManualSalesInvoice.ConfirmInvoices
- OperationStepsByProductionOrder.StartOverview
- PhysicalBreakdown.CreateStructure
- ProductionOrder.GenerateHandlingUnitForLineSideLabelingV2
- ReceivedPurchaseInvoice.Cancel
- SalesOrder.CreditCheck
- SalesOrderLine.GenerateProductionOrder

### 2025.06

- CustomerClaim.GenerateLinesFromDocument
- HandlingUnit.PrintLabels
- InventoryCommitment.Generate
- ItemIssueByWarehouseAndPeriod.StartDetail
- ItemIssueByWarehouseAndPeriod.StartOverview
- MaintenanceSalesOrder.Cost
- MaintenanceSalesOrderLine.Cost
- PlannedActivity.CreateMaintenancePlan
- ProductionOrderOperation.GetMaximumQuantityToReportComplete
- QuarantineInventoryDispositionLine.ProcessV2
- QuarantineInventoryHandlingUnit.ProcessV2
- ReceiptLine.StartAwaitingDirectMaterialSupply
- SalesOrder.StartIntakeWorkbench
- SalesOrderLine.LinkContract
- SalesPrice.StartCalculate
- ServiceContract.Create
- ServiceContractConfigurationLine.Create
- ServiceContractInstallment.SwitchStatus
- ServiceContractInstallment.Transfer
- StandardCosts.CalculateSingleItem
- Tax.GetDefaultsGoods
- User.ConvertToRuntime
- WarehouseInspection.Print

### 2025.07

- Asset.StartAdjustAssets
- HandlingUnit.StartDetail
- HandlingUnit.StartOverview
- HandlingUnit.StartPrintLabels
- InventoryCommitment.ConsumeFromBuffer
- ProductionOrder.RemoveHandlingUnit
- ProductionOrder.RemoveUnusedHandlingUnits
- ProductionOrder.SendHandlingUnitsToWarehouse
- ProjectContractDeliverable.StartMonitor
- PurchaseOrderMaterialSupplyLines.StartOverview
- QuarantineInventoryDispositionLine.GenerateInboundAdvice
- ReceiptLine.CorrectFinalReceipt
- ReceiptLine.Reverse
- SalesOrderLine.Split
- StandardCosts.ActualizeCostCompStructureForConfigurableItem

### 2025.08

- ItemByItemCodeSystem.StartOverview
- ItemCodeSystem.StartOverview
- NonConformanceReport.StartWorkbench
- OutboundOrderLine.StatusOverviewStartDetail
- OutboundOrderLine.StatusOverviewStartOverview
- ProductionOrderAdvice.Transfer
- ProductionOrderOperation.StartReportCompleted
- Series.StartOverview
- TestData.Update
- WarehouseInspection.StartDetail
- WarehouseInspection.StartMultiMain
- WarehouseInspection.StartOverview

### 2025.09

- Call.TransferToPlannedActivity
- Call.TransferToServiceQuote
- HandlingUnit.GlobalBlock
- JobShopBillOfMaterial.StartPrintWhereUsedComponent
- ProductionOrder.MoveRejectedMaterialToQuarantineV2
- ProjectedShipment.Generate
- Task.StartMultiMain
- WorkOrder.Close
- WorkOrderActivity.SignOff

### 2025.10

- BOD.PublishWithBatch
- BRA.WarehouseReceipt.CreateBrazilianInvoice
- BRA.WarehouseReceipt.RevertBrazilianInvoice
- Calendar.CalculateDaysFromHours
- EngineeringBillOfMaterial.StartOverview
- ExportControl.Check
- JobShopBillOfMaterial.GetSourceEngineeringRevision
- Kanban.StartGenerate
- PlannedPeggingRelations.RetrievePurchaseRelated
- PlannedPeggingRelations.RetrieveSalesRelated
- ProductionOrder.CompressAutomatedQueueTime
- ProjectBudget.GetTotalBudgetAmountOfProject
- ProjectContract.GetTotalAmountContractLines
- SanctionList.Check
- SerializedItem.ChangeStatus
- SerializedItem.GetWarrantyEndDate
- ServiceContract.StartMultiMain
- ServiceContractConfigurationLine.GetContractCoverageDetails
- ServiceContractInstallment.Cancel
- WarehouseItemRentalInventory.StartDetail
- WarehouseItemRentalInventory.StartMultiMain
- WarehouseItemRentalInventory.StartOverview

### 2025.11

- AdjustmentOrder.GenerateHandlingUnitsV2
- BRA.BrazilianReceipt.MatchReceiptLinesToFiscalDocByWarehouseReceiptArray
- InstallationGroup.ViewStructure
- ItemWarehouse.StartDetail
- ItemWarehouse.StartOverview
- Job.Activate
- MaintenanceNotification.FollowUpUnderCurrentOrder
- OrderPegging.StartOrderDetails
- ProductionOrderOperation.GetDefaultQuantityToComplete
- PurchaseInvoice.StartMultiMain
- PurchaseOrderAdvice.Confirm
- PurchaseOrderAdvice.StartDetail
- PurchaseOrderAdvice.StartOverview
- PurchaseRequisition.RejectApprovalRecord
- PurchaseRequisition.Submit
- PurchaseSchedule.StartMultiMain
- ServiceContractChange.Create

### 2025.12

- AdditionalFile.Upload
- Documents.StartOverview
- ItemBySiteAndOffice.StartCreate
- ItemIssueByPeriod.StartDetail
- ItemIssueByPeriod.StartOverview
- ItemLotSerialTracking.GetChildren
- ItemLotSerialTransactions.Get
- ItemSerial.CreateOrUpdate
- MaintenanceSalesOrderLine.ItemReceipt
- OutboundAdvice.StartGenerateV2
- OutboundOrderLine.UndoAdvice
- OutboundOrderLine.UndoRelease
- OutboundRun.UndoAdvice
- OutboundRun.UndoRelease
- PlannedOrder.GetUnconfirmedDemand
- ProductionOrder.Close
- ProjectActivityBudgetOverview.StartOverview
- ProjectBudget.StartGenerateControlData
- PurchaseOrder.RecalculateLandedCosts
- QuarantineInventory.StartDisposition
- RequestForQuoteResponse.Accept
- RequestForQuoteResponse.Reject
- SalesOrderInstallments.StartMultiMain
- SalesOrderLine.LinkOrUnlinkInstallationGroup
- SalesOrderLine.ReleaseManualActivity
- Shipment.NotifyAutomaticOutboundProcess
- Shipment.ProcessInvoice

### 2026.01

- AssemblyOrder.Delete
- HandlingUnit.Remove
- Inventory.DetermineCompanyOwnedValueDependingOnValuationBasis
- ItemOrderPlan.StartSessionForOrder
- OutboundOrderLine.CreateAdviceV2
- ProductionBillOfMaterial.Explode
- ProductionOrder.ReportProductV3
- ProductionOrder.ReportProductWithSerialsV3
- ProductVariant.Validate
- PurchaseOrder.StartPrintReminders
- QuarantineInventoryDisposition.Process
- SalesContract.StartMultiMain
- SalesOrder.Rush
- ServiceContract.UnpackObjectReference
- Specification.StartDetail
- WarehouseOrder.CalculateReceiptDate

### 2026.02

- EffectivityUnit.CreateObjectKey
- InspectionOrder.PrintSamples
- ItemCosting.StartDetail
- ItemCosting.StartOverview
- ItemOrderPlan.GetV2
- Kanban.GenerateOrders
- MaintenanceSalesOrderLine.Cancel
- SalesOrderLine.CalculateCostPriceByProject
- StandardCosts.CalculateAndGetStandardCostV3

### 2026.03

- BillableLine.Compose
- EngineeringItem.StartDetail
- EngineeringItem.StartOverview
- EngineeringItemRevision.StartMultiMain
- Inventory.CheckAndRepair
- Logger.Close
- Logger.Log
- Logger.Open
- PlannedPRPOrders.Generate
- ProjectOrderLineBalance.StartOverview
- WhereUsedJobShopListOfMaterial.StartOverview

### 2026.04

- Activities.StartOverview
- Asset.Adjust
- Asset.Dispose
- BRA.BrazilianInvoice.CancelInvoice
- BRA.BrazilianInvoice.CopyInvoice
- CitiesByCountry.StartDetail
- FreightPlanning.MoveShipmentLine
- Lot.Delete
- ProductionOrderOperation.StartMoveRejectedEndItemToQuarantine
- Project.GetCostRateTask
- PurchaseOrderChangeRequest.Cancel
- QuarantineInventoryDispositionLine.Split
- ReceiptDetails.StartOverview
- RequestForQuote.PrintRequestForQuote
- RequestForQuote.StartCancel
- SalesOrderChangeRequest.Cancel
- SerializedItem.StartDetail
- SerializedItem.StartOverview
- Shipment.ConfirmASN
- WarehouseOrder.GenerateOutboundAdvice
- WarehouseOrder.StartAutomaticProcessing

### 2026.05

- AssemblyOrder.RefreshAndFreeze
- BRA.Department.GetEstablishmentCode
- BRA.TaxForecast.Calculate
- Common.CalculateTranportationDate
- Installment.Close
- MaintenanceSalesOrder.StartDetail
- MaintenanceSalesOrder.StartMultiMain
- MaintenanceSalesOrder.StartOverview
- PrepackingAdvice.StartGenerateProposal
- PrepackingAdvice.StartMultiMain
- PrepackingAdvice.StartOverview
- PrepackingAdvice.StartPrintPackingSheet
- PriceBooks.StartOverview
- Printing.CloseOpenReports
- ProductVariant.ValidateConstraint
- ProjectBudget.GenerateControlData
- PurchaseRequisition.StartMultiMain
- PurchaseRequisitionPrepareConversion.StartMultiMain
- ReceiptLine.Correct
- ReceiptLine.RemoveHandlingUnits
- ShipmentPlanning.StartOverview

### 2026.06

- AdditionalFile.Download
- AssemblyOrder.Move
- AssemblyOrder.Swap
- AssemblyProductVariant.GenerateStructure
- EngineeringBillOfMaterial.StartPrint
- Installment.StartMultiMain
- Invoice.StartInvoicing360
- InvoiceToBusinessPartner.StartOverviewBalances
- OpenEntry.StartMultiMain
- PlannedActivity.StartOverview
- ProcessingOptionSet.CheckOptionNames
- ProcessingOptionSet.Read
- ProductionOrder.DecreaseQuantityOperations
- ProductionOrder.SendAndReceiveHandlingUnitsInWarehouse
- ProjectPCS.StartCopyCustomizedProductStructureToCustomizedStructure
- ProjectPCS.StartCopyCustomizedProductStructureToStandardStructure
- ReceivedProductionBillOfMaterial.Process
- WorkOrderIncomingSubassembly.Confirm
- WorkOrderOutgoingSubassembly.Confirm

### 2026.07

- Call.SetStatusToInProcess
- IntercompanyTradeOrder.CalculateCommercialPrice
- Inventory.ProcessVariances
- InventoryCommitment.Cancel
- OutboundAdvice.Pick
- Personalization.Copy
- PlanItem.GetHorizonDate
- PrepackingAdvice.Create
- ProjectPCS.CalculateCostV2
- PurchaseOrder.Approve
- PurchaseOrderLine.Generate
- PurchaseOrderLine.RecalculatePriceAndDiscounts
- SalesOrder.CheckInventory
- SalesQuote.Print
- ServiceOrderMaterialCosts.RecalculatePriceDiscounts

### 2026.08

- Call.CreateInvoice
- Call.ViewInvoice
- CommissionAgreement.StartMultiMain
- EngineeringItem.GenerateByMBC
- IntercompanyTradeOrder.CalculateEstimates
- ItemBySite.StartCreate
- OutboundAdvice.Release
- PickingList.PickAdvice
- PickingList.PickMission
- PickingList.PickRun
- PickingList.StartDetail
- PickingList.StartOverview
- PlanItem.GetInboundLeadTime
- PlanItem.GetOrderLeadTime
- PlanItem.GetOutboundLeadTime
- PrepackingAdvice.GenerateProposal
- PriceBook.InitiateChangeRequest
- ProductionOrder.CancelV2
- ProductionOrderOperation.MachineWorklistReportProduct
- ProjectPCS.CalculateItemSalesPrice
- ProjectPeggedInventory.CalculateExcess
- SalesContractLine.Activate
- SalesContractLine.Deactivate
- SalesOrder.Cancel
- SalesPrice.CalculateSurchargesV2
- WarehouseOrder.Activate
- Warehousing.GetExpectedTransactionAmount

### 2026.09

- Activity.Create
- AssemblyOrder.GenerateSupplyMessages
- BlockingReason.Release
- BOD.GetAddressFromLocationNode
- CrossDockOrder.UpdateSystemPriority
- EngineeringBillOfMaterial.ApproveMBCByProduction
- EngineeringBillOfMaterial.FinalizeMBC
- EngineeringBillOfMaterial.GenerateMBCCopyData
- EngineeringBillOfMaterial.ProcessMBC
- HandlingUnit.CommitInventory
- InboundOrderLineActivity.Create
- InstructionsByOperationStep.StartOverview
- MachineOperation.StartDetail
- MachineOperation.StartOverview
- OrderRelation.GetLatestSequenceNumber
- ProductionPlanning.StartDetail
- ProductionPlanning.StartOverview
- ProductVariant.CreateWithFeaturesOptions
- ProductVariant.ExecuteSubstitutionConstraint
- ProductVariant.InitializeOptionSet
- ProductVariant.InitializeVariant
- Project.UpdateWorkAuthorizationStatus
- RentalOrder.StartMultiMain
- RentalOrderMaterialCosts.ConfirmScrap
- Requirements.StartOverview
- ServiceOrder.PrintServiceOrder
- ServiceOrderMaterialCosts.ConfirmScrap
- ShipmentNotice.SetToScheduledManually
- ShipmentPlanning.StartOverviewV2
- WarehouseOrder.Print

### 2026.10

- ConfigurableStructure.StartDetectLoops
- ConfigurableStructure.StartValidateRevision
- ConfigurableStructure.StartValidateVersion
- ConfigurableStructures.StartDetail
- ConfigurableStructures.StartOverview
- CrossDockOrder.Generate
- CrossValidationRule.Rebuild
- CrossValidationRule.SetStatus
- GenericAssemblyBillOfMaterial.StartDetectLoops
- GenericAssemblyBillOfMaterial.StartReplaceItem
- GenericAssemblyBillOfMaterial.StartValidate
- Installment.Approve
- Invoice.ReprintV2
- PlannedSubcontractingOrder.Transfer
- ReceivedPurchaseInvoice.Validate
- RentalOrderMaterialCosts.RemoveItem
- ServiceOrder.ProcessProFormaInvoices
- ServiceOrderMaterialCosts.RemoveItem
- ServiceQuote.PrintQuoteDocuments
- ShipmentLine.ReturnNotShippedGoods
- TradeNoteReceivable.StartProcess

### 2026.11

- GenericAssemblyBillOfMaterial.ActualizeVersion
- GenericAssemblyBillOfMaterial.CreateNewVersion
- PlanItem.GetPlanningHorizonDate
- PlanItem.StartATPHandlingV2

## Process Extensions

The following table shows the Infor LN Cloud releases in which new Process Extensions were released:

### 2019.07

- PurchaseSelfBilledInvoice.CustomCompose
- SalesCheckInventory.CustomSorting

### 2019.08

- SalesOrder.SkipReleaseToWarehousing

### 2019.09

- InventorySearchEngine.CheckHandlingUnit
- InventorySearchEngine.CheckStockPoint
- ProjectBudget.SkipGenerateServiceOrders
- ProjectControl.SkipPrint
- ProjectNetworkPlanning.SkipGenerate
- ProjectPlannedPurchaseOrder.SkipApprove
- ProjectPlannedWarehouseOrder.SkipApprove
- ProjectStructuralElement.SkipGenerate
- ProjectTransaction.SkipProcess
- PurchaseOrder.SkipApprove
- PurchaseOrder.SkipPrintPurchaseOrder
- PurchaseOrder.SkipReleaseToWarehousing
- QualityNonConformance.SkipPrint
- SalesOrder.SkipApprove
- SalesOrder.SkipPrintAcknowledgement

### 2019.11

- HandlingUnitBuilding.CustomShipmentLine

### 2020.03

- OutboundAdvice.SkipPrint
- PaymentReceipt.CustomXMLHandling
- QualityFailureDocument.SkipPrint

### 2020.05

- BillOfLading.SkipShipmentLine
- PlannedServiceActivity.SkipSwitchStatus

### 2020.06

- ProductVariant.AdjustPrice
- SalesQuote.SkipPrintSalesQuote

### 2020.08

- PurchaseOrder.SkipPrintPurchaseInvoice
- PurchaseOrderLine.SkipPrintPurchaseInvoice
- PurchaseOrderLine.SkipPrintPurchaseOrder

### 2020.09

- FactoryTrackQuery.QueryExtend
- ProductVariant.AddToCPQConfiguration

### 2020.11

- FreightLoadBuilding.CustomBuildingConstraints
- FreightShipmentBuilding.CustomBuildingConstraints
- LoadBuilding.CustomBuildingConstraints
- ShipmentBuilding.CustomBuildingConstraints

### 2020.12

- BOD.PublishCustomBOD

### 2021.03

- Invoice.SkipPrintInvoice
- ProductionOrder.SkipReportComplete

### 2021.04

- OutboundAdvice.SkipGeneratePickingList
- OutboundAdvice.SkipReleaseAdvice
- OutboundOrderLine.SkipGenerateAdvice
- Procurement.DefaultPurchaseOffice
- SalesCheckInventory.DefaultInventoryShortageOption
- SalesCheckInventory.SkipShortageLine
- ServiceOrderActivity.SkipComplete

### 2021.05

- SalesOrderLine.SkipPrintAcknowledgement

### 2021.06

- StandardCost.SkipActualize

### 2021.08

- InvoiceFromBP.SkipPrintAgingAnalysis
- InvoiceFromBP.SkipPrintOpenEntries
- InvoiceToBP.SkipPrintAgingAnalysis
- InvoiceToBP.SkipPrintOpenEntries

### 2021.10

- Invoice.CustomXMLInvoicing

### 2021.11

- BillableLine.CheckConfirmedAllowed
- SelfBilledInvoiceLine.SkipMatching

### 2021.12

- Pricing.RetrievePriceAndDiscounts
- PurchaseOrderLine.SkipPrintPurchaseOrderReminder
- QualityManagement.DefaultStandardTestProcedure

### 2022.02

- ShiftsByWorkcell.SkipCompleteShift

### 2022.03

- EffectivityUnit.CheckInterchangeable

### 2022.04

- PickingMission.HandleAfterGenerate
- ProjectPCS.SkipClose
- StorageMission.HandleAfterGenerate

### 2022.05

- InvoiceFromBP.SkipWriteOffPaymentDifference

### 2022.06

- ItemMasterPlan.SkipCopyToScenario

### 2022.09

- AssemblyOrder.SkipGenerate
- StandardCost.SkipCalculate

### 2022.10

- ComposeShippingStructure.CommandHandling
- ExtendedTimeManagement.ProcessingInstructions
- ProductionOrder.PrintCustomReports
- PurchaseOrderGenerate.SuppressCommingleLine

### 2022.11

- AssemblyOrder.SkipDelete

### 2022.12

- AssemblyPart.SkipBackflush
- PlannedOrder.SkipTransfer

### 2023.04

- PriceBookLine.SkipGlobalUpdatePricesViaPriceBooks
- SalesContractDeliveryScheme.SkipGenerateOrder
- SalesOrderGenerate.SuppressCombiningOrderLines

### 2023.05

- ExceptionMessagesByItem.SkipPrint

### 2023.06

- ItemMasterPlan.SkipCopyToScenarioForPeriod
- PaymentAdvice.SkipPaymentBatch
- ProductionOrder.SkipCalculateIntermediateResults
- ProductionOrder.SkipClose

### 2023.09

- InterestInvoiceAdvice.SkipPrintAdvice
- Invoice.SkipSelectInvoiceForReminding
- InvoiceToBP.SkipSelectForInterestInvoices
- OpenItem.SkipSelectForInterestInvoices
- PaymentAdvice.SkipPrintPaymentAdvice
- ReminderAdvice.SkipPrintReminderAdvice
- ReminderLetter.SkipPrintReminderLetter
- ReservedApprovedCommissionsRebates.SkipPrint
- SalesOrder.SkipPrint
- SalesOrderLine.SkipPrint

### 2023.10

- ConfigurableItem.SkipCompileConstraints
- Invoice.CustomComposingCriteriaMet
- PerformInventoryValuation.SkipItem
- Tax.FlexibleFieldsVertex
- Tools.SkipScrap

### 2023.11

- PackingList.SkipPrint

### 2023.12

- Common.DeterminePurchaseType
- ProjectContract.SkipProcessProFormaInvoices
- ProjectContract.SkipTransferToInvoicing

### 2024.01

- PaymentAdvice.CustomAuditPayments
- SalesOrderGenerate.DetermineOrderForCombining
- Statement.SkipPrintStatements

### 2024.02

- DeliveryNote.SkipPrint
- HandlingUnitLabel.SkipPrint

### 2024.03

- ProductVariant.AddCustomCPQIntegrationParameters

### 2024.04

- BrazilianInvoice.CustomXMLInvoicing
- DirectDebitAdvice.SkipDirectDebitBatch
- InboundAdvice.SkipPutawayAdvice
- PurchaseOrderGenerate.HandleCombining
- ReceivedCustomerOrderProcess.Validate

### 2024.05

- RequestForQuoteConvert.CustomSorting
- TPOP.SkipPlannedTransaction

### 2024.06

- PurchaseOrder.SkipPrintReceivableInvoice
- PurchaseOrderLine.SkipPrintReceivableInvoice
- SalesQuoteLine.SkipPrintSalesQuote
- TransferOrderPlanning.CustomSorting

### 2024.07

- GenerateHandlingUnits.SkipOrderLine
- IntegrationTransaction.SkipPrint
- ReceivableInvoice.SkipPrintSpecification

### 2024.08

- HandlingUnit.CustomIdentifier
- Invoice.SkipPrintOpenEntries
- TransferOrderPlanning.CustomOrderSeries

### 2024.09

- BillableLine.SkipComposing
- ComposeShippingStructure.CustomCommand
- InventoryAgingAnalysis.Custom
- InventoryAgingAnalysis.Skip
- ProductionOrder.SkipGenerateSubcDocuments

### 2024.10

- BankStatement.CustomMatchOnExternalInvoice
- IntrastatTransaction.CheckBlockedForReporting
- ProductVariant.SkipValidate
- ProjectPCS.SkipGlobalStatusUpdate

### 2024.11

- InvoiceFromBP.SkipCalculateAgingAnalysis
- InvoiceToBP.SkipCalculateAgingAnalysis
- SalesOrder.DetermineSupplier

### 2024.12

- SalesOrderLine.AllowSaveInCaseOfInsufficientATP
- TargetPrices.SkipCalculateOrderLines
- WarehouseOrder.CreateNewHeader

### 2025.01

- AccountsReceivable.GetEffectiveDateForAging
- Common.GetItemSignalInfo
- Invoice.CombineAdvancesIsAllowed
- SalesOrderLine.SkipGenerateProjectStructure

### 2025.02

- BlockedSalesOrderOrSalesOrderLine.SkipFirmRelease
- BlockedSalesOrderOrSalesOrderLine.SkipSoftRelease
- Invoice.SkipCreditNoteLinking
- OrderPlanning.GetCustomRouting

### 2025.03

- CreditTransferBOD.CustomMapping
- CycleCountingData.SkipDelete

### 2025.04

- GLTransaction.SkipPrintStandardAuditFileTax
- LocationSearchEngine.CheckLocation
- PackingSlip.SkipPrint
- PlannedInventoryTransaction.SkipPrint
- ProductVariant.SkipDeleteOrExpireJSBOM
- PurchaseConsumption.SkipGenerateSelfBillingInvoice
- ServiceOrderLine.HandleActionsAfterUpdateReceipts
- ServiceQuote.SkipProcess

### 2025.05

- InventoryCommitments.SkipGenerate

### 2025.06

- GeneratePlan.SkipOrderLine
- Pricing.DefaultPriceDateType

### 2025.07

- ProductStructure.SkipCopyComponent
- SalesOrderInvoiceLine.SkipReleaseToInvoicing

### 2025.08

- IND.EInvoice.GetCustomizedJSON
- ProductionOrder.SkipMaterialLineAggregation
- ProductionOrder.SkipPrintMaterialShortage
- TaxDeclaration.GetXMLTextTagContent

### 2025.09

- PlannedOrder.SkipPrint
- Procurement.HandleGeneralLedgerCodeDefaulting
- ProductionOrder.SkipCreateOrderGroup
- ReservedCommissionsRebates.SkipReservationApproval
- SalesContract.SkipPrintSalesContract

### 2025.10

- Batch.SkipSelectForAuditBatches
- SalesOrder.SkipAdditionalCostSet
- TUR.eTransport.GetData

### 2025.11

- CommissionsRebates.SkipCalculate
- CycleCountingOrderLine.SkipGenerate
- GenerateFieldChangeOrderLine.SkipSerializedItem
- Sales.SkipRetroBilledAdviceLineGeneration

### 2025.12

- CommissionsRebates.DetermineTaxCountry
- SalesOrderLine.AllowDisplaySelectedContract

### 2026.01

- PlannedOrder.SkipConfirmOrder
- ProductionOrder.SkipWarehouseForMaterialShortage

### 2026.03

- AdvanceInstallment.SkipPrintZ5Reporting
- IntegrationTransaction.GetMaxNumberOfDocumentLines
- InterimResults.SkipGenerateRevenues
- MaterialSupplyLines.HandleCombining
- PlannedPRPOrder.SkipGenerate

### 2026.04

- BRA.BrazilianFiscalReceipt.Validate
- BRA.BrazilianInvoice.DirectProcessCriteria
- BRA.BrazilianInvoice.Validate
- Dataset.CustomOutputFormat
- PurchaseInvoice.SkipMatchApproveByRange
- Tax.ExternalTaxProviderHandling

### 2026.05

- BRA.WarehouseOrder.GetBusinessPartner
- IntercompanyTradeOrder.SkipReleaseToInvoicing
- PurchaseRequisition.SkipConvert

### 2026.06

- Invoice.SkipAssignToFactor
- Pricing.HandleInputOverruling
- ProductVariant.SkipRefreshCPQ
- PurchaseOrder.HandleAdditionalActionsAfterPrint
- PurchaseOrderGenerate.InputOverruling
- ReceiptLine.HandleConfirm

### 2026.07

- ItemOrderPlan.CustomActions
- OutboundAdvice.AllowOverdelivery

### 2026.08

- Invoice.SkipSelectForDirectDebit
- Invoice.SkipSelectForPayment
- Receipt.HandleReceive
- SalesQuote.SkipApprove

### 2026.09

- IntegrationTransaction.SetDocumentSeries
- PurchaseOrder.SkipPrint
- PurchaseOrderLine.SkipPrint
- Sales.AdditionalCostAmountCalculationSkipLine
- Sales.DetermineAdditionalCostSet
- ShipmentLine.SkipFreezeConfirm
- StandardCost.UseAlternativeScheme

### 2026.10

- InventorySearchEngine.OverruleQuantityToAdvise
- Invoice.SplitRevenueByComponent
- OCM.SkipObjectApproval
- ProjectPCS.CustomNumberSeries
- StockPointBlocking.BeforeCheck
