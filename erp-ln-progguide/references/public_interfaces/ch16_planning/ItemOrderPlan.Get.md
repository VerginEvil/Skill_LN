...
use:    ItemOrderPlan.GetV2
This function calculates the Item Order Plan for the specific
...
iShowOrderDistribution  If true and period length is set to 'Detail',
then the output will be on the most
detailed level (order distribution).
Else output is aggregated on Order Position level.
Values: true, false
Output:
