...
>
> Source: Infor LN Public Interfaces & Process Extensions Reference Guide (Cloud), pp. 281-283

```baan
DLL:   tdextslsapi
This function is available from     2020.03 (KB2111387  ).
Syntax: long Sales.GetQuoteSettings(
...
... )
Usage:        Expl:   This function retrieves settings for sales quotes. Individual
...
(settings table tdsls081) depending on the specified input
...
Example: when sales quote series and step size for sales quote
lines are needed, the function must be called as follows:
if Sales.GetQuoteSettings(
|* Fixed arguments:
company,                                                --> input
sales office,                                           --> input
site,                                                   --> input
...
|* Variable arguments:
"sesq",                                                 --> input
series.for.sales.quotes,                                --> output
"sssq",                                                 --> input
step.size.for.sales.quote.lines) <> 0 then                               --> output
|* Error, do something
Exception.Delete(exception.id)
endif
Alternatively, the function can also be called with "sesq.1"
and "sssq.1", respectively, which refers to the mnemonics at
parameter level.
Pre:    None
Post:   None
Input:
