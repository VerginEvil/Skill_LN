...
>
> Source: Infor LN Public Interfaces & Process Extensions Reference Guide (Cloud), pp. 495-496

```baan
DLL:   tdextpcgapi
This function is available from     2020.03 (KB2111387  ).
Syntax: long Pricing.GetPurchaseSettings(
...
... )
Usage:        Expl:   This function retrieves purchase pricing settings.
...
(settings table tdpcg080) depending on the specified input
...
number to the field                      -mnemonic. Example: "clsi(2)", "rrsi(7)".
Example: when target price currency and price book number group
are needed, the function must be called as follows:
if Pricing.GetPurchaseSettings(
|* Fixed arguments:
company,                                                --> input
site,                                                   --> input
