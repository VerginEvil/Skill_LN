...
```baan
Handle customer determined input for pricing.
This process extension is available from 2026.06 (KB3670627).
...
quantity and price date to a value other than the defaulted value based
on LN logic.
This quantity (if valid) or price date (if valid) is then used in all
pricing flows where prices and discounts are retrieved
using the order quantity or price date.
