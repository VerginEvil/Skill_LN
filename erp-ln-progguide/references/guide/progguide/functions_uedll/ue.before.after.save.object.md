...
In case a DAL is present, this hook is executed just before the [after.save.object()](../functions_dal/after.save.object.md) hook.
In case no DAL is present, this hook is executed just before the `main.table.io:after.write:` and `main.table.io:after.rewrite:` UI script sections.
In case of a 'stand-alone'[db.update()](../functions_db_operations/db.update.md) or [db.insert()](../functions_db_operations/db.insert.md), this hook is executed just after the database operation itself.
...
function extern long ue.before.after.save.object(long mode)
{
        on case mode
        case DAL_NEW:
        		| Do some actions just before the standard
