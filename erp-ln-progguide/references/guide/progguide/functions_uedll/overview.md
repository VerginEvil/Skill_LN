...
- [db.delete()](../functions_db_operations/db.delete.md)  and the User Exit DLL for the related table is present, the UEDLL will be loaded prior the database operation. This means that there is no need to have a DAL or a session in order to make use of the UEDLL.
