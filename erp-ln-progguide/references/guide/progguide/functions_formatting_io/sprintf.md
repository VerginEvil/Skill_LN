...
| `[ void` | `... ]` |  One or more values to be placed in the format [Implicit conversion](../3gl_features/type_conversions.md#implicit_type_conversion) of the input value is performed from its original type to the type expected according to the corresponding substitution symbol.  |
...
| *0* | An optional flag which triggers leading zero padding in the string representation of numeric values. This is primarily intended for the substitution types for numeric values: %d, %e, %f, %g, %o, %lo, %x, and %lx. The zero padding fills the available space in the formatted string as specified by the *width* element. The '0' flag applies to other *types* as well. However, in such cases the leading zero padding simply replaces the leading white space padding; no attempt is made to do the leading zero padding just before the digits of the numeric part of the string. |
...
- A special substitution type behaves similar to the edit$() function: %@<format>@.

These substitution types are explained in more detail in the following subsections.
[Implicit conversion](../3gl_features/type_conversions.md#implicit_type_conversion) of the input value is performed from its original type to the type expected according to the substitution symbol.
...
| %h | name of month | (‘Jan’ - ‘Dec’) (abbreviations) |
| %H | name of month | (‘January’ - ‘December’) |
| %a | name of day | (‘Sun’ - ‘Sat’) (abbreviations) |
...
The value that is substituted for this symbol must specify a number of days since 01-01-0001.
...
The value that is substituted for this symbol must specify a [UTC](../functions_date_time_zones/overview.md#utc) long format value of which only the local time will be used.
