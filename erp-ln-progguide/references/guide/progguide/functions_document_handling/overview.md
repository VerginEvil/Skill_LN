...
- In the Attribute Mapping session on the right, a default mapping is filled. Check this mapping. Now document handling for the selected table via the Document Hub is available for extensions.     The API will be exposed via include bic_dms which must be included in the declaration hook of the extension with #include <bic_dms>
