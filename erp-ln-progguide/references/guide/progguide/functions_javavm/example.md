...
Below is an example of a custom listener. The listener is installed from within the Infor Enterprise Server program using the following program fragment (for clarity, all checks on return codes are omitted. This is not recommended in a real-life program!):
```
