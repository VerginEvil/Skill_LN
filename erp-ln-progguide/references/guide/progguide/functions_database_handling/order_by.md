...
<order by clause>
    ::= ORDER BY <order by item> [ { , <order by item> }... ] [ WITH RETRY [REPEAT LAST ROW] ]
...
Adding a [WITH RETRY clause](with_retry.md) does not have an impact on the working of the ORDER BY clause. It modifies the result table of the WHERE clause after a jump to a db.retry.point(). See section [WITH RETRY clause](with_retry.md).
