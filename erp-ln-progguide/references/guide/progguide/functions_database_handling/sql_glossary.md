...
The degree of a table is the number of columns of that table. An array column counts as one column. The _compnr column also counts as one column. Other pseudo columns (e.g. _index1) are expanded and each of their underlying columns counts as one column.
...
A select target is the program variable that is used to store the result value of a column of a SQL statement.
