...
- A list of AND conditions can contain only one condition that includes <table>._compnr. So, the following construction is possible: `WHERE <table>._compnr = 100 AND <condition>` but the following is not possible: `WHERE <table>._compnr = 100 AND <table>._compnr = 200` It is possible to include multiple <table>._compnr._compnr conditions that specify the company number of different table references. For example, the following construction is possible: `WHERE <table1>._compnr = 100 AND <table2>._compnr = 200`
