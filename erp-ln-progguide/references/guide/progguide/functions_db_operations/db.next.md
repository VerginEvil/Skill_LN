...
## Syntax:
`function long db.next( long table_id, [ long lock ] )`

## Description
This reads the next record from a specified table. It uses the current index to search the table. If there is no current record, the function returns an error message.
