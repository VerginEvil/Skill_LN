...
## Syntax:
`function long db.gt( long table_id, long lock )`

## Description
This reads from a specified table the record whose key value is greater than a certain predefined value. Before calling *db.gt()*, you must assign the required value to the key field. For example:
```
