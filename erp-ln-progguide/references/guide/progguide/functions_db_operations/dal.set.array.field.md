...
- If the array depth is 4 and the field depth is 3, the last element of the array will not be used.  This function is a shorthand for the following (pseudo) code:
```
...
- Argument *value* must be an array and memory must have been allocated. So passing a based array for which no alloc.mem() has been done yet, is not allowed.     Note  This function does not update any dependent fields itself, nor does it check the field's value! These actions are done during a save.
