...
## Syntax:
`function long db.lt( long table_id, [ long lock ] )`

## Description
This reads from a specified table the record whose key value is less than a certain predefined value. Before calling *db.lt()*, you must assign the required value to the key field. For example:
```
...
If the [resource](../misc/bshell_resources.md) Link> *mle_all_data_languages* has the value 0, then the default behavior is to retrieve only the current data language.
