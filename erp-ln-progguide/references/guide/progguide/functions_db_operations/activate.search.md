...
When a new record is being added to the database, the 4GL engine normally executes a search action in the main table after input to the field(s) of the primary index. If the key values entered belong to an existing record in the main table, the 4GL engine displays all the fields of that record and stops adding the new record. Otherwise, the user must fill the remaining fields; then the record is added to the main table.
