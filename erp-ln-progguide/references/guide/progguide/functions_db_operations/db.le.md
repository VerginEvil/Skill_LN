...
## Syntax:
`function long db.le( long table_id, [ long lock ] )`

## Description
This reads from a specified table the record whose key value is less than or equal to a certain predefined value. Before calling *db.le()*, you must assign the required value to the key field. For example:
```
