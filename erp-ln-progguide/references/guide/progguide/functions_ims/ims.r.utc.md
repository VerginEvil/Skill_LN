...
## Description
Reads a specified number of bytes from the byte array. The bytes read from the byte array are interpreted as the two’s complement representation of a signed integer value.

## Arguments
| | | |
|---|---|---|
| `long` | `bytearray` |  The byte array identifier that is returned by ims.openvba or ims.openfba. This function reads a number of bytes from the byte array as specified by *byte.count*.  |
| `[ long` | `byte.count ]` |  Optional argument specifying the number of bytes to be read from the byte array. Allowed values are 4, 5, and 8. Any other value is interpreted as 4. Default value is [ByteCountOfUtc](../functions_date_time_zones/overview.md#ByteCountOfUtc). According to the value of this argument, the (big endian) byte layout of the [Utc32 mode](../functions_date_time_zones/overview.md#Utc32), the [Utc40 mode](../functions_date_time_zones/overview.md#Utc40), or the [Utc64 mode](../functions_date_time_zones/overview.md#Utc64) is used.  |
...
| >= -(256^ *byte.count*)/2 and < (256^ *byte.count*)/2 | The numerical value of the bytes read from the supplied byte array. Typically, this is a (non-negative) [UTC](../functions_date_time_zones/overview.md#utc) long format value stored earlier by the function [ims.w.utc()](ims.w.utc.md), or prepared by any other means. This function does not perform any value check. |
