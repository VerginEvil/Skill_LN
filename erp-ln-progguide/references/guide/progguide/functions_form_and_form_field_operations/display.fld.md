...
## Description
Use this function to display a specified field of a specific occurrence on a form. Usually, the [4GL engine](../glossary/glossary.md#fourgl_engine) displays fields automatically, either when the form is opened or when data in a related input field is changed. However, there are situations when you need to use these functions to display fields. For example, to display stand-alone fields, to display fields that are not in the input field TAB sequence, or to send data to the form during batch processes.
| | |
|---|---|
