...
1. In the include section of the main application, add the following: `#include <bic_plcm> | all PLan Chart Manager defines.`
1. In the main function (or where appropriate) call *plcm.init()*
1. Create the PLan Chart by using the different plcm.* functions.
1. Call *plcm.start()* to start the Plan chart on the client.
