...
## Syntax:
`function long cipher.decrypt.finalize( long Handle, string data.out, long data.out.length )`
...
| | | |
|---|---|---|
| `long` | `Handle` |  A valid handle obtained from a previous call to [cipher.decrypt.initialize()](cipher.decrypt.initialize.md). Also, some data should have been decrypted using [cipher.decrypt.update()](cipher.decrypt.update.md).  |
| `string` | `data.out` |  The output buffer.  |
