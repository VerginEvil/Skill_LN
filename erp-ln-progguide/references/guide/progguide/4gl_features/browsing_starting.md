...
In order to understand how to start browse list sessions from the application directly, it is good to see how the 4GL engine does this.

## Using the 4GL engine
The 4GL engine uses the following predefined variables while starting a browse list session:
| | |
|---|---|
...
Based on these variables the 4GL engine basically performs the following:
Parent session:
```
...
```
Note: Here you can see that the 4GL engine is not able to handle the situation where a complete index should be returned. Function *start.session()* will return e.g. "tccom100._index2" and this value is stored in the current field. The application has to correct this in the *after.zoom* section. See [Exporting variables](browsing_exporting.md) for more information.
Browse list session:
When the browse list session starts the 4GL engine imports the *attr.zoomindex* variable. Based on that variable values, the session starts up with the correct key.

## Using start.session directly
Based on how the 4GL engine starts a browse session, the application can do this generally as follows:
Parent session:
```
