...
A string variable can be declared as fixed. The intention is that the [character length](data_types.md#character length) of its value always equals the [character capacity](data_types.md#character capacity) of the variable. When a string value with a lower character length is assigned to such a string variable, it is space-padded to reach the desired character length. For example:
```
...
- When a value is assigned to a substring, then any [zero-padding](null_characters_in_strings.md#zero-padding) in the surrounding string *before* the substring is replaced by space-padding.

In the following example, each ␣ represents one space character.
```
...
- [at.base(basic_value [, position, ...], based_variable [, length, ...])](../functions_variables_based/at.base1.md)

- [at.base(process_id, basic_variable_name [, position, ...], based_variable [, length, ...])](../functions_variables_based/at.base2.md)
