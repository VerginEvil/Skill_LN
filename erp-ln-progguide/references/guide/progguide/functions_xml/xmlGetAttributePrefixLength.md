...
## Syntax:
`function long xmlGetAttributePrefixLength( long node, long attributeNr )`

## Description
Get the length of the prefix of an attribute of the specified node.
...
| | |
|---|---|
| >= 0 | Success; The length of the prefix. |
...
long length
length = xmlGetAttributePrefixLength xml_envelope, 1 )

| length now contains 4
```
