...
## Syntax:
`function long xmlAllocAttributeURI( ref string basedString, long node, long attributeNr, [ const string default.value ] )`
...
| | | |
|---|---|---|
| `ref string` | `basedString` |  *basedString* is the return argument to receive the URI. It must be declared *based*.  |
...
| | |
|---|---|
| 0 | Success; The *basedString* contains the URI if the *node* is valid, the *attributeNr* is valid and the attribute is in a namespace. It contains the *default.value*, or when this is omitted, the empty string otherwise.  |
...
string URI based
long ret
ret = xmlAllocAttributeURI( URI, xml_envelope, 1 )
