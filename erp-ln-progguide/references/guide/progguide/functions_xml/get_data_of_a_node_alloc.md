...
## Syntax:
`function long xmlAllocData( ref string basedString, long node, [ const string default.value, const string separator ] )`
...
| | | |
|---|---|---|
| `ref string` | `basedString` |  *basedString* is a based string buffer (either or not multibyte) which will implicitly be allocated to the correct size (see [alloc.mem()](../functions_memory_operations/alloc.mem.md)) and will be filled with the data.  |
...
long	xmlGetData( long node, ref string data, const string separator, [ const string separator ] )
