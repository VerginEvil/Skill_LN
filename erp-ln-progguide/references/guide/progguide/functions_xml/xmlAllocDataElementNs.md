...
## Syntax:
`function long xmlAllocDataElementNs( ref string basedString, long node, void namespaceOrURI, const string name, [ const string default.value, const string data.separator, const string element.separator ] )`
...
| `[ const string` | `default.value ]` |  *default.value* is the value that will be stored in *basedString* when the function fails. When *default.value* is not specified, an empty string is used.  |
...
string data based
long ret
ret = xmlAllocDataElementNs( data, xml_stockprice, "http://www.example.org/stock", "StockName", "Def" )
