...
## Syntax:
`function long xmlAllocURI( ref string basedString, long node, [ const string default.value ] )`
...
| | |
|---|---|
| 0 | Success; The *basedString* contains the URI if the *node* is valid and is in a namespace. It contains the *default.value*, or when this is omitted, the empty string otherwise.  |
...
long ret
string URI based
ret = xmlAllocURI( URI, xml_envelope )
| ret now contains 0
