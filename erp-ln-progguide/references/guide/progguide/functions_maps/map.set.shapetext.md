...
## Syntax:
`function void map.set.shapetext( long rpid, string text )`

## Description
Show a text instead of a sequence number in this route point. A maximum of two characters of the text are shown.

## Arguments
| | | |
|---|---|---|
| `long` | `rpid` |  A route point object id returned by function [map.add.point()](map.add.point.md).  |
| `string` | `text` |  The text to be shown inside the shape.  |
