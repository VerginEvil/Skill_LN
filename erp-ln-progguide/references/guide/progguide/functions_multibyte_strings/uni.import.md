...
| `const string` | `source$` |  The source string that must be converted. Its contents are considered to be encoded according to a [byte serialized UTF-16](../misc/utf16.md) encoding scheme. As its contents are not encoded in TSS, it is desirable that this argument is of type string, rather than type multibyte string.  |
| `[ long` | `sb_flag ]` |  This optional argument specifies the byte-order of the *source$* string. Default is UNI_MSB_ORDER.  |
...
## Related topics
- Inverse operation: [uni.export()](uni.export.md)
