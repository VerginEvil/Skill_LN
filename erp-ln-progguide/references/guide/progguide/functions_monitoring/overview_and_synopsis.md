...
The messages sent to the url are conform the *InfluxDb* line protocol, the datamodel used in the API is based upon the syntax of these messages.
The url the messages are sent to is expected to be the http-listener of the locally installed *Telegraf*, but can belong to any proces which is able to handle *InfluxDb* line protocol (e.g. *InfluxDb*). When another proces is targeted, or when *Telegraf* listens on a different port, an alternative url can be configured. The messages are sent unencrypted, when encryption is required this can be provided by *Telegraf*.
The messages are sent asynchronously, so the flow of the session is not interrupted. Success of triggering an event indicates the message will be sent. When the actual sending fails, a bshell message will be generated, but this falls outside the flow of the sending session. When messages are generated faster as they can be sent, they will be combined into larger messages going to the same url.
...
- Metrics A metric is a non-identifying attribute of an event. It can be of type string, long, boolean or double. For each event of a certain class the value for a metric must be provided. When no value for a defined metric is provided, the metric will not be represented in the message sent to the monitoring system.

After a class has been prepared, events of this class can be triggered. The values for the metrics to be represented in the message generated, have to be provided.
...
| *monitor_url* | *localhost:8186* | The URL the events will be sent to. |
