...
## Syntax:
`function void aud.put.new.field.value( long pid, string variable.name, long selection.id, long table.id, long field.id, long element, [ bool endian ] )`

## Description
Use aud.put.new.field.value to retrieve the value of a field from the new value string. The retrieved value is put into the variable supplied by the application.
...
Status <> "X" and ( *action.type* = "I" or *action.type* = "U")
