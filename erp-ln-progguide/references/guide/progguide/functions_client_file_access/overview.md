...
- to show an external HTML page in the browser

Some functions are still present which can only be used when using WebUI or BW but which are not supported for LN UI. You can use these function for operations:
...
Note  Several functions are only supported for Baan Windows and/or for WebUI client and not for LN UI client. Therefore these functions are deprecated.
