...
- indicate the query extends for this session  From every 4GL Session the function is.rbhp.mode() can be called to determine if the session was started from a Role Based Home Page drillback. In case a session is started from such a drillback, the function rbhp.get.parameter() can be used to retrieve the parameters passed in the drillback URL.
