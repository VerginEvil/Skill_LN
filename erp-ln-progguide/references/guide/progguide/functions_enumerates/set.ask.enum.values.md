...
After execution of *ask.enum()*, the specified enum values are cleared again. So you have to call *set.ask.enum.values()* again before calling *ask.enum()*. This also happens when the 4GL engine calls *ask.enum()*.
