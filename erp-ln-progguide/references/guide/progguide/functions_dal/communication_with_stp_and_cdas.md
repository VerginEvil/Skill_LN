...
Infor Enterprise Server provides the following functions to enable the [4GL engine](../glossary/glossary.md#fourgl_engine) and external applications to retrieve and display DAL error messages:
...
- [dal.set.error.message(), dal.set.warning.message(), dal.set.info.message()](../functions_message_handling/dal.set.error.message.md)

You use *dal.set.error.message()* to specify a message to be displayed when the DAL returns an error (that is, a negative value). This function is called in the DAL script and not in the UI script.
