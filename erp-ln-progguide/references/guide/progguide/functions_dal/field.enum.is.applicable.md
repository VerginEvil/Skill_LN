...
## Description
Use this hook to indicate whether a certain enum constant is applicable. In case the enum constant is not applicable the 4GL engine will not show it in the field's list box, so the end-user cannot select it.
...
## When called
- At the time the 4GL engine determines the enum masks of enum fields: this is done just before calling the *before.display.object* and the *when.field.changes* sections
- At the time of checking the field's value    Note  This hook only applies to enum fields.
If this hook does not exist, it is assumed that the enum constant is applicable.
The 4GL engine will show the following message to the end-user in case the field value is the enum value being checked and this is not allowed : Change the value of the %1$s field.
