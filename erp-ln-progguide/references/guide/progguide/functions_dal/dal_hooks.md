...
When a user issues a command to access the database, the 4GL engine loads the DAL DLL for the object being accessed and calls the hooks to perform the integrity checks. Provided that a DAL script exists for an object set being accessed, the 4GL engine always ensures that the hooks in the DAL are called at the appropriate times.
