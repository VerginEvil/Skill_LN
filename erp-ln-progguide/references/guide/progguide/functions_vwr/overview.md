...
- The 3GL Script must include bic_vwr and must implement the main() function. In this main function a call must be done to the functions: vwr.init() and vwr.start().   For sending an update message from a navigation session to a viewer session, the PRCM mechanism can be used.
