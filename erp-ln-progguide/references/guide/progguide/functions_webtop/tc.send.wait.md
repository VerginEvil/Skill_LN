...
## Syntax:
`function long tc.send.wait( ref long processno, string bucket )`

## Description
This function sends a bucket message to a specified process and waits for a reply. It notifies the WebUII/LN UI as well about this action. The receiving process must send a reply by using [send.bucket()](../functions_interprocess_communication_bshell/send.bucket.md) or [send.wait()](../functions_interprocess_communication_bshell/send.wait.md). Note that the waiting process cannot receive any message string contained in the reply.
...
Notes  *tc.send.wait()* is used to synchronize two processes. When a reply is sent with *send.bucket()* the processes appear to run simultaneously.
...
| Note: first argument of tc.send.wait is a ref!
tc.send.wait(parent_process, "I am waiting for a reply")
```
