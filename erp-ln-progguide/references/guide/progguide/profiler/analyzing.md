...
-
- Look at the functions that are the most expensive in the Flat Profile. If the cost is significant it may be useful to try to optimize the implementation of these functions.
-
- Look in the Query Summary for SQL statements that have a long run time, or for SQL statements that have much higher number of fetch operations than expected.
-
- Walk through the Call Graph from the `main()` function. Look for details in child functions that contribute significantly to the cost of the session. Remember your knowledge of the business logic in the session and try to understand whether it is really necessary to do the things you see and to do these things the number of times you see them being done.
-
