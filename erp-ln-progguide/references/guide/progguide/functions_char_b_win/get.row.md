...
## Syntax:
`function long get.row( )`

## Description
*Deprecated.* This function is only supported for character-based windows and its usage is therefore deprecated.
This retrieves the number of the row where the cursor is currently positioned. The function uses [get.cp()](get.cp.md), so it is slower than that function.
