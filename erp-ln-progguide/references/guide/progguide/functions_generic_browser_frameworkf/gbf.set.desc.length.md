...
## Syntax:
`#include <bic_gbf>`
`function long gbf.set.desc.length( long desc.length )`

## Description
The following functions are actually defines and they are defined as:
`gbf.set.desc.length(desc.length): gbf.set.key.desc.length(0, desc.length)`
...
| | | |
|---|---|---|
| `long` | `desc.length` |  The new description length.  |
