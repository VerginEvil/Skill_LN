...
## Syntax:
`function long bms.receive.buffer( ref string broadcast, ref long no.bytes, [ long level, long wait.flag, ref string mask ] )`

## Description
Use this to receive a broadcast message. The function can retrieve a complete record, including ‘\0’ characters.

## Arguments
| | | |
|---|---|---|
| `ref string` | `broadcast` |  This returns the received broadcast message.  |
| `ref long` | `no.bytes` |  This returns the number of bytes in the broadcast message.  |
| `[ long` | `level ]` |  This optional argument specifies a message level. The following values are available: 0: the function returns the first broadcast message sent to the process > 0: the function returns the first broadcast message sent with the specified level < 0: the function returns the first broadcast message sent with a level less than or equal to the absolute value of the specified level  |
