...
- ` [sjv.nullable()](sjv.nullable.md)`- specifies the array is nullable   You can specify the type of the elements using one of the following functions:
...
- ` [sjv.tuple()](sjv.tuple.md)`- each element is a JSON array of fixed length is expected of which the elements may be of different types
- ` [sjv.domain()](sjv.domain.md)`- each element is a JSON value that has a type that is in accordance with the type of the domain   Example:
```
