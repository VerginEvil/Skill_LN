...
- enum keywords in case of an enum domain; applies to JSON string values; the string value must be one of the allowed enum keywords

By default, the value is optional (i.e., does not have to appear in the JSON), can also be empty ("", 0, 0.0), but may not be null. The following aspects can be specified:
...
- [sjv.filled()](sjv.filled.md)- specifies the value may not be "", 0.0 or 0

For string type domains you can also specify one of the following formats:
