...
Note  The functions [end()](end.md), *stop()*, and [exit()](exit.md) (without an *exitvalue* argument) in 3GL programs, and the function *stop()* in 4GL programs, all have the same effect. That is, the program ends and returns control to the application. The database is not updated.
