...
## Syntax:
`function long aud_get_info_seq_hdr_size( long seqid, long seqno )`
...
This function returns the size (in bytes) of the header of a specified sequence file, as stored in the associated info file. You can use the return value to allocate space for the sequence header before retrieving it with [aud_read_info_seq_hdr()](aud_read_info_seq_hdr.md) or [aud_read_seq_hdr()](aud_read_seq_hdr.md).
...
Note  You can also use [aud_get_hdr_size()](aud_get_hdr_size.md) to retrieve the size of the sequence header. The value returned by both functions is normally the same.
