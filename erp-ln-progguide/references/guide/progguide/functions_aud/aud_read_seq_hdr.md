...
## Syntax:
`function long aud_read_seq_hdr( long seqid, ref string seqhdr )`

## Description
This retrieves the header information from a specified sequence file. You can access information in the sequence header with the [Macros - sequence header](macros_sequence_header.md).
Use [aud_read_info_seq_hdr()](aud_read_info_seq_hdr.md) to read the sequence header from the info file instead of the sequence file. The information retrieved by both functions is normally the same.
