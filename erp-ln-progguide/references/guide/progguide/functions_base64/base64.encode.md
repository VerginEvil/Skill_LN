...
## Return values
The size of the base64-encoded output is returned. If this value is greater than the [byte capacity](../3gl_features/data_types.md#byte capacity) of data.out, then no output at all is written to data.out.
To compute the size of the base64-encoded output, only the [byte capacity](../3gl_features/data_types.md#byte capacity) of the binary input must be known, the actual input itself is not needed. For each three binary input bytes, four base64-encoded output characters are produced. For any (one or two) remaining binary input bytes at the end, also four base64-encoded output characters are produced.
So, to compute the size of the base64-encoded output, divide the [byte capacity](../3gl_features/data_types.md#byte capacity) of the binary input by 3, and if that is not an integer, round up to the next integer. Multiply the result by 4.
