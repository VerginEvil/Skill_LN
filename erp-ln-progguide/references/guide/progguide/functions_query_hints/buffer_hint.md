...
A buffer hint can be used for a query that selects a large set of records and when we need to process all of these records. In this case a large set of records has to be transferred from the driver to the client. It is more efficient to transfer a few large sets than transferring a large number of small sets. However using a large buffer may not be beneficial in all cases. If a query selects a large set of records but we actually use only the first few records then buffering many rows may in fact be disadvantageous.
