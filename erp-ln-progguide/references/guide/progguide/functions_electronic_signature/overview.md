...
1. The Reason for Signing (mandatory) by choosing one of the entries from a pre-defined list of reasons.
1. (Optional) User Comments to provide additional clarification on the action to be performed.
1. A valid Username and Password (mandatory)  Only after successfully signing the Signature Request Dialog will the action be performed.
...
- long tcgen.dll3000.store.signed.document(domain ttesg.docm i.document.type, long i.keyfields.xml, long i.document.xml) - this function must return 0 for the Document Type and also return the current values of the document (in XML) identified by i.keyfields.xml. The following functions are available to create the Document XML (i.document.xml):
