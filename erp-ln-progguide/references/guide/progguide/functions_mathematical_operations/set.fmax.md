...
## Syntax:
`function void set.fmax( ref void variable, [ string domain ] )`

## Description
This sets a specified string variable to its maximum value.
For standard variables, use [set.max()](set.max.md) instead.
For setting the minimum value, use [set.fmin()](set.fmin.md) and [set.min()](set.min.md).
...
long value
set.fmax(value) | value contains 2147483647
```
