...
## Syntax:
`function void set.fmin( ref void variable, [ string domain ] )`

## Description
This sets a specified string variable to its minimum value.
For standard variables, use [set.min()](set.min.md) instead.
For setting the maximum value, use [set.fmax()](set.fmax.md) and [set.max()](set.max.md).
...
long value
set.fmin(value) | value contains -2147483648
```
