...
## Syntax:
`#include <bic_json>`
`function long Json.putLong( [long] json_array, long idx, long value )`

## Description
Convenience function for directly putting a 3GL long value at the specified index in a JSON array. The 3GL long value is first converted to a JSON number value, which is then put into the array.
...
| `long` | `value` |  The 3GL long value for which a JSON number value is created, which is then put into the array.  |
