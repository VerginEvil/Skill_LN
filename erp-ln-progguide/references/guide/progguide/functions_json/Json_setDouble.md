...
## Syntax:
`#include <bic_json>`
`function [long] Json.setDouble( long json_object, const string key, double value )`

## Description
Convenience function for directly associating a 3GL double value with a key in a JSON object. The 3GL double value is first converted to a JSON number value, which is then associated with the specified key.
...
| `double` | `value` |  The 3GL double value for which a JSON number value is created, which is then associated with the key.  |
