...
## Syntax:
`#include <bic_json>`
`function long Json.insertDouble( [long] json_array, long idx, double value )`

## Description
Convenience function for directly inserting a 3GL double value at the specified index in a JSON array. The 3GL double value is first converted to a JSON number value, which is then inserted into the array. All values at the specified index and above are shifted one position to the right.
...
| `double` | `value` |  The 3GL double value for which a JSON number value is created, which is then inserted into the array.  |
