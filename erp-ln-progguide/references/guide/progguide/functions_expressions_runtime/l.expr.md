...
If the resulting integer value is greater than the maximum value 2^( BitCountOfLong-1) - 1 of the signed BitCountOfLong-bit range, then that maximum value is returned.
If the resulting integer value is less than the minimum value -2^( BitCountOfLong-1) of the signed BitCountOfLong-bit range, then that minimum value is returned.
