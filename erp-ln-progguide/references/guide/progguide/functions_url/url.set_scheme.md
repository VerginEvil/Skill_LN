...
## Syntax:
`#include <bic_web>`
`function void url.set_scheme( long url_instance, const string scheme )`

## Description
Sets the scheme of a URL instance, e.g. `http` or `ftp`.

## Arguments
| | | |
|---|---|---|
| `long` | `url_instance` |  A URL instance.  |
| `const string` | `scheme` |  A scheme, like `"https"`.  |
