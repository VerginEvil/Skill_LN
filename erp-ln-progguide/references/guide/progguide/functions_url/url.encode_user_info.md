...
## Description
Encodes a user info string according to RFC 3986, so it can be safely used in a URL. A user info string has the format `user[:pwd]` (the `:pwd` part is optional).
