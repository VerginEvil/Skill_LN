...
1. Start the Chart Manager Application Data session (ttchm1500m000)).
1. Create a new application record.
1. Define the option sets (Chart-Specific Options).
1. Create a default window setting (Window Preferences).
1. Start the Chart Manager from the Chart Manager Application Data session (ttchm1500m000).
1. Create the chart types required to present the data.  By using the Business Chart Manager functions described in this document, you can create a client application that starts the Business Chart Manager and that sends data to and receives data from the Business Chart Manager. For further details on using the Chart Manager, see the session help.
