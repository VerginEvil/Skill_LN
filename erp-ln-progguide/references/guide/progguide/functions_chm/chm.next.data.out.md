...
## Syntax:
`function long chm.next.data.out( long option, ref double series_value, ref double category_value, ref long data_number, ref double data_value, ref long footnote_no )`

## Description
*Deprecated.* This function is only supported for Baan Windows and its usage is therefore deprecated.
You use [chm.first.data.out()](chm.first.data.out.md) to retrieve information about the first data point in the current chart. You use *chm.next.data.out()* to retrieve information about the next data point in the same chart. To retrieve information about all subsequent data points, you can place this function in a loop that executes as long as the return value is CHM_OK.
...
| | | |
|---|---|---|
| `long` | `option` |  Use this argument to indicate whether the function retrieves data for the absolute next data point or for the next data point that has been modified. The possible values are: CHM_ALL_DATA CHM_CHANGED_DATA  |
