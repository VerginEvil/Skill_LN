...
Casting an integer value to type string (char or varchar) results in the string representation of the integer value. If the target is too short, then the result is [undefined](sql_glossary.md#Undefined). Casting a string value to type integer results in the integer representation of the string value. The string value must consist of an optional minus sign ('-') immediately followed by a consecutive sequence of digits only. The string may be enclosed by spaces. Otherwise the result is [undefined](sql_glossary.md#Undefined).
